...
- Ollama and LM Studio health/model probes, local generation calls, provider egress allowlist checks with redirect blocking, safe provider response metadata, disabled external placeholder routing, and scored provider routing.
...
- Provider system: Ollama and LM Studio runtime calls exist with endpoint allowlist enforcement, redirect blocking, safe provider telemetry, and a disabled external placeholder; production external adapters, secure credential storage, rate-limit/retry handling, and streaming generation remain.
