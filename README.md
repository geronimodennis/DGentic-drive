...
- Provider system: Ollama and LM Studio runtime calls exist with endpoint allowlist enforcement, redirect blocking, bounded retry/backoff for retryable generation failures, safe provider telemetry, a disabled external placeholder, and an OpenAI-compatible external adapter that is disabled until configured with an HTTPS base URL, model allowlist, and credential env-var reference; bound provider approval records, encrypted credential storage, circuit-breaker/cost handling, and streaming generation remain.
