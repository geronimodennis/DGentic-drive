...
Status: backend MVP sprint surface active; Sprint 15 production identity, secrets, and network guardrails is in progress, with BL-009a through BL-009c implemented.
...
- Production/staging bearer-token authentication gate with route capability groups for tasks, auth-token management, filesystem, CLI, providers, agents, memory, tools, sessions, logs, startup fail-closed validation when auth is enabled without tokens, and persisted generated bearer tokens with salted PBKDF2 hashing, one-time raw token return, rotation, revocation, expiry, safe audit events, and operator-id actor binding for persisted tokens.
...
- Security/auth: production and staging auth default-on route capability gates, persisted hashed bearer tokens with rotation, revocation, expiry, operator-id approval actor binding, persisted external credential references, and provider-call network/domain guardrails exist; broader persisted identity profiles, encrypted local credential vaulting, external secret manager adapters beyond env references, full audit actor propagation, and non-provider network approval workflows remain.
...
- Full production identity management, encrypted local credential storage, external secret-manager adapters beyond env references, and non-provider network approval workflows: planned for remaining Sprint 15 follow-up; persisted generated token lifecycle APIs, external credential references, and provider-call network/domain guardrails are implemented as Sprint 15 slices.
