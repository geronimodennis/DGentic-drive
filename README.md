...
Status: backend MVP sprint surface active; Sprint 12 provider productionization is complete and Sprint 13 memory production lifecycle is active, with the SQL-backed lifecycle policy foundation implemented.
...
- Memory and retrieval: memory record storage, text/tag search, SQLAlchemy metadata index services, metadata CRUD routes, deterministic semantic retrieval fallback, SQLite vector backend abstraction, stored vector retrieval, lifecycle metadata, preview/apply lifecycle APIs, and retrieval API tests exist; pgvector integration, actual compression/summarization execution, scheduled lifecycle jobs, broader performance validation, and source-attribution/scoring improvements remain production follow-up work.
