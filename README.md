...
- Tool runtime: local tool generation, execution, governance, reliability tracking, runtime reliability policy automation, generated-tool SQL registry auto-registration, registry duplicate preflight checks, deprecated registry exclusion, permission conflict fail-closed checks, bound approval records for approval-required tool execution outside development/test mode, output/log redaction for tool execution, usage tracking, and registry routes exist; stronger OS/process sandbox isolation, richer version migration policy, and production-safe dependency isolation remain.
