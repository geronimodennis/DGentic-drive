...
- Security/auth: production and staging auth default-on route capability gates and persisted hashed bearer tokens with rotation, revocation, expiry, and operator-id approval actor binding exist; broader persisted identity profiles, encrypted credential storage, external secret manager integration, and full audit actor propagation remain.
...
- Full production identity management, secret management, encrypted credential storage, and network/domain guardrails: planned for Sprint 15; persisted generated token lifecycle APIs are now implemented as the first Sprint 15 slice.
