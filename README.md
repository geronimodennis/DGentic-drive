...
- `POST /tasks/orchestrations`, `GET /tasks/orchestrations`, `GET /tasks/orchestrations/{run_id}`, `POST /tasks/orchestrations/{run_id}/advance`, `POST /tasks/orchestrations/{run_id}/cycle`, `PATCH /tasks/orchestrations/{run_id}/tasks/{task_id}`, `POST /tasks/orchestrations/{run_id}/tasks/{task_id}/recover`, `POST /tasks/orchestrations/{run_id}/blockers/{blocker_id}/resolve`, and `POST /tasks/orchestrations/{run_id}/close` for backend-managed task graphs with dependency scheduling, role-boundary decisions, blockers, follow-ups, retry escalation, manual/security blocker resolution, and Definition of Done close gates.
...
Status: backend MVP sprint surface active; Sprint 14 autonomous agent orchestration is active, with BL-008a through BL-008h implemented.
...
- Agent orchestration: lifecycle contracts, documentation, backend-managed task graphs, machine-readable role-boundary decisions with canonical declared-path validation, dependency scheduling with redacted dependency-output context handoff, explicit agent lifecycle reconciliation cycles, orchestration-bound filesystem write checks, orchestration-bound CLI action checks, orchestration-bound generated-tool approval/execution checks for active agent context, retry escalation, recoverable blocker handling, admin-reviewed manual/security blocker resolution, blockers/follow-ups, bounded scheduling passes, actor attribution when auth is enabled, and DoD close gates exist; a fully autonomous background execution loop, automatic backlog/progress mutation from run events, durable shared memory coordination, production multi-agent scheduling, and UI/operations surfacing remain.
...
- Full autonomous backlog management, automatic sprint document updates, and end-to-end autonomous execution inside the backend runtime: remaining Sprint 14 work.
