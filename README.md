...
- `POST /tasks/orchestrations`, `GET /tasks/orchestrations`, `GET /tasks/orchestrations/{run_id}`, `POST /tasks/orchestrations/{run_id}/advance`, `PATCH /tasks/orchestrations/{run_id}/tasks/{task_id}`, and `POST /tasks/orchestrations/{run_id}/close` for backend-managed task graphs with dependency scheduling, role-boundary decisions, blockers, follow-ups, retry escalation, and Definition of Done close gates.
...
Status: backend MVP sprint surface active; Sprint 14 autonomous agent orchestration is active, with BL-008a through BL-008c implemented.
...
- Backend orchestration runs for multi-agent task graphs, including dependency-aware scheduling into sub-agent briefs, machine-readable role-boundary decisions, orchestration-bound filesystem write checks, orchestration-bound CLI action checks for active agent context, blocker and follow-up records, retry escalation, persisted run history, and Definition of Done evidence gates before closeout.
...
- Tool runtime: local tool generation, execution, governance, reliability tracking, runtime reliability policy automation, generated-tool SQL registry auto-registration, registry duplicate preflight checks, explicit same-name version migration with SQL reliability reset, deprecated registry exclusion, permission conflict fail-closed checks, bound approval records for approval-required tool execution outside development/test mode, output/log redaction for tool execution, local-only dependency import isolation, process-group launch and timeout cleanup hardening, usage tracking, and registry routes exist; full OS/filesystem/network sandbox isolation, parallel multi-version SQL registry rows, and production package/dependency lifecycle management remain.
- Agent orchestration: lifecycle contracts, documentation, backend-managed task graphs, machine-readable role-boundary decisions with canonical declared-path validation, dependency scheduling, orchestration-bound filesystem write checks, orchestration-bound CLI action checks for active agent context, retry escalation, blockers/follow-ups, bounded scheduling passes, actor attribution when auth is enabled, and DoD close gates exist; a real autonomous execution loop, automatic backlog/progress mutation from run events, shared context/memory coordination, runtime binding for tool actions, production multi-agent scheduling, and UI/operations surfacing remain.
