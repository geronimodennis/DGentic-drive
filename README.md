...
- Ollama and LM Studio health/model probes, local generation calls, provider egress allowlist checks with redirect blocking, bounded retry/backoff for retryable generation failures, safe provider response metadata, disabled external placeholder routing, and scored provider routing.
...
- Provider system: Ollama and LM Studio runtime calls exist with endpoint allowlist enforcement, redirect blocking, bounded retry/backoff for retryable generation failures, safe provider telemetry, and a disabled external placeholder; production external adapters, secure credential storage, circuit-breaker/cost handling, and streaming generation remain.
