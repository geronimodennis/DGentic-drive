...
Set `DGENTIC_MANAGED_SETTINGS_FILE` to opt into an organization-managed JSON settings file. The file must contain a top-level `settings` object, for example `{"settings":{"auth_enabled":true,"network_domain_policy":{"default_mode":"deny","rules":[]}}}`. Managed values override `.env` and process environment values, unknown or malformed fields fail closed during settings load, and `GET /settings/effective` reports redacted values with `default`, `environment`, `managed`, or `derived` source labels plus a SHA-256 digest for the managed file. This is the first backend precedence foundation; persisted command, hook, plugin, and recipe policy-source override contracts remain follow-up work.
...
Status: backend MVP sprint surface active; Sprint 15 production identity, secrets, and network guardrails is in progress, with BL-009a through BL-009aa implemented.
...
- DGentic-native plugin bundles beyond manifest discovery/trust and declarative command recipe activation, richer managed settings policy-source controls, and guarded git/PR execution automation beyond read-only checkpoints: planned across remaining Sprint 15 security follow-ups plus Sprint 16 UI, Sprint 17 CLI/VS Code, and Sprint 18 CI/observability work after the Claude Code repository study.
