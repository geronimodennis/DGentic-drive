...
Status: backend MVP sprint surface active; Sprint 14 autonomous agent orchestration is closed, with BL-008a through BL-008q implemented.
...
- Ollama, LM Studio, and disabled-by-default OpenAI-compatible external provider contracts, with health/model probes for local runtimes, local generation calls, NDJSON streaming for OpenAI-compatible runtimes, provider-scoped egress allowlist checks with redirect blocking, bounded retry/backoff and in-process circuit breakers for retry-exhausted generation failures, safe provider response metadata, HTTPS-only credential-safe external adapter configuration with persisted external credential references or env-var fallback, deferred credential/header resolution until transport, single-use bound external provider approvals, bounded provider/model pricing estimates, role-aware provider/model routing, and scored provider routing.
...
- Security/auth: production and staging auth default-on route capability gates, persisted hashed bearer tokens with rotation, revocation, expiry, operator-id approval actor binding, and persisted external credential references exist; broader persisted identity profiles, encrypted local credential vaulting, external secret manager adapters beyond env references, and full audit actor propagation remain.
...
- Full production identity management, encrypted local credential storage, external secret-manager adapters beyond env references, and network/domain guardrails: planned for Sprint 15; persisted generated token lifecycle APIs and external credential references are implemented as early Sprint 15 slices.
