...
- `POST /cli/recipes`, `GET /cli/recipes`, `GET /cli/recipes/{recipe_id}`, `PATCH /cli/recipes/{recipe_id}`, `POST /cli/recipes/{recipe_id}/preview`, `POST /cli/recipes/{recipe_id}/execute`, `POST /cli/recipes/{recipe_id}/approvals`, and `POST /cli/recipes/{recipe_id}/runs` for persisted command recipe CRUD, safe parameterized expansion, policy preview, synchronous execution, approval creation, and asynchronous runs through the existing CLI policy, approval, runtime, redaction, and audit contracts.
- `POST /cli/git/checkpoints` for read-only git workflow readiness snapshots before commit, push, or PR preparation, including root-bounded repository inspection, staged/unstaged/untracked summaries, diff stats, protected-file and secret-shaped staged-addition blockers, test-evidence requirements, protected-branch blockers, authenticated requester binding, audit metadata, and a checkpoint digest. `POST /cli/git/commit-approvals` re-runs the commit checkpoint, requires a fresh matching ready digest, validates a single-line non-secret commit message, and creates a workflow-bound pending CLI approval for the generated `git commit -m ...` command. `POST /cli/git/push-approvals` re-runs the push checkpoint, requires a fresh matching ready digest, a configured upstream remote URL digest, local commits ahead, no remote-tracking commits behind, and creates a workflow-bound pending CLI approval for `git push`. These approval-creation endpoints do not run `git add`, `git commit`, `git push`, `gh`, or any network PR operation; execution still requires the existing CLI approval approve/execute flow, which revalidates the git workflow binding before claiming the approval.
...
- `GET /plugins`, `GET /plugins/{plugin_id}`, `PATCH /plugins/{plugin_id}/trust`, `POST /plugins/{plugin_id}/command-recipes/preview`, `POST /plugins/{plugin_id}/command-recipes/install`, and `POST /plugins/{plugin_id}/command-recipes/disable` for backend-only plugin manifest discovery, safe redacted metadata summaries, SHA-256 manifest digests, persisted explicit trust decisions, and digest-bound declarative command recipe activation without importing or executing plugin code.
...
Status: backend MVP sprint surface active; Sprint 15 production identity, secrets, and network guardrails is in progress, with BL-009a through BL-009ad implemented.
...
- Managed settings precedence foundation with an opt-in `DGENTIC_MANAGED_SETTINGS_FILE`, managed-over-environment runtime override semantics, fail-closed malformed/unknown field handling, redacted per-field source reporting, and an admin-gated `GET /settings/effective` endpoint.
- Command recipe execution contracts with persisted `command-recipes.json` records, safe template placeholders, bounded parameter values, secret-shaped text rejection, preview expansion with command policy decisions, recipe execute/approval/run endpoints, principal-bound requester metadata when auth is enabled, usage auditing, and no recipe-level `approved` bypass outside the existing CLI approval model.
- Git workflow safety checkpoints with a shell-free, read-only `POST /cli/git/checkpoints` endpoint for commit/push/PR readiness metadata, root-bounded git inspection, content-sensitive checkpoint digests, protected branch/file and secret-shaped staged-addition blockers, test-evidence requirements, no-network PR warnings, authenticated requester binding, and command-policy protection so configured-safe `git` rules cannot downgrade mutating git commands. Commit and push approval creation through `POST /cli/git/commit-approvals` and `POST /cli/git/push-approvals` is also implemented for fresh ready checkpoint digests, generated pending CLI approvals, and execution-time git workflow revalidation before approval claim.
- Managed policy surface locks through `managed_policy_locks` in `DGENTIC_MANAGED_SETTINGS_FILE`, allowing deployment-managed read-only mutation gates for CLI policy rules, hook policy rules, command recipes, plugin trust decisions, and plugin command recipe activation while leaving read/preview/evaluation routes available.
...
- CLI integration: approvals, single-use bound approval IDs, policy rules, status polling, chunked output polling, supervision metadata, stale-running reconciliation, cancellation, conservative post-restart orphan termination for single-worker restart recovery, context-aware rules, matched policy review metadata, safe approval review API contracts, workflow-bound approval metadata for git commit/push revalidation, environment controls with startup/preload injection override blocking, top-level `cmd` AutoRun and PowerShell profile/prompt suppression, bare executable workspace/PATH trust checks, POSIX execution parity for simple `cmd /c` wrappers, cwd-aware approval policy evaluation, reviewer routes split to the `approvals` capability when auth is enabled, broader Windows/POSIX wrapper parsing regressions, command-name escape decoding, launcher payload inspection, protected state-file escape checks, read-only path operand boundary checks, explicit executable path boundary checks, command-specific path argument hardening for configured-safe `git`, `npm`, `pnpm`, `yarn`, and `uv` directory flags, mutating-git downgrade protection for configured-safe `git` rules, read-only git workflow checkpoints, checkpoint-bound git commit/push approval creation, and nested shell startup hardening checks exist; PR execution automation, direct git workflow runners beyond approval-bound CLI execution, full process adoption/resumable output after restart, production multi-worker lease semantics, full shell emulation/OS sandboxing, and the interactive approval UI remain.
...
- Security/auth: production and staging auth default-on route capability gates, persisted operator identity records with direct and group-inherited effective capability assignments, persisted operator groups, persisted hashed bearer tokens with rotation, revocation, expiry, operator-id approval actor binding, authenticated audit actors across the main API-triggered execution/mutation surfaces, persisted external credential references, encrypted local credential-vault references with supplied-key rotation, shell-free external-process credential resolver adapters, provider-call network/domain guardrails with bound approval records, bound filesystem approval records, active-task verification for caller-supplied orchestration agent context across CLI, generated-tool, provider, and network approval surfaces, method-aware CLI and filesystem approval reviewer capability separation, generated-tool Python socket network policy enforcement, plugin manifest trust controls, plugin command recipe activation gates, backend hook-policy records for command/filesystem/network guardrail escalation, managed settings precedence foundation, managed policy surface locks, explicit CLI executable path rootDir enforcement, configured-safe command path-argument hardening, nested shell startup hardening checks, and secret-shaped metadata redaction for identity/token/credential/plugin trust/hook labels exist; richer production identity workflows beyond operator groups, managed KMS integration, first-class secret-manager adapters, web retrieval network enforcement, OS-level egress isolation, and per-record policy-source controls remain.
...
- DGentic-native plugin bundles beyond manifest discovery/trust and declarative command recipe activation, per-record managed policy-source controls beyond coarse surface locks, and guarded git/PR execution automation beyond checkpoints and checkpoint-bound commit/push approval creation: planned across remaining Sprint 15 security follow-ups plus Sprint 16 UI, Sprint 17 CLI/VS Code, and Sprint 18 CI/observability work after the Claude Code repository study.
...
- Full production identity management beyond operator groups, managed KMS integration, first-class external secret-manager adapters, web retrieval network enforcement, OS-level egress isolation, and richer managed policy-source controls: planned for remaining Sprint 15 follow-up; persisted operator profiles, persisted operator groups with capability inheritance, generated token lifecycle APIs, external credential references, encrypted local credential-vault references with supplied-key rotation, shell-free external-process credential resolver adapters, provider-call network/domain guardrails with bound approval records, generated-tool Python socket network policy enforcement, plugin manifest trust controls, plugin command recipe activation gates, command recipe execution contracts, task-scoped active orchestration context checks, method-aware CLI approval reviewer capability separation, explicit CLI executable path boundary enforcement, top-level shell startup hardening, bare executable workspace/PATH trust checks, configured-safe command path-argument hardening, nested shell startup hardening checks, managed settings precedence foundation, and identity/token/credential/plugin-trust metadata redaction are implemented as Sprint 15 slices.
