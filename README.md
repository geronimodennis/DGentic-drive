...
- Provider system: Ollama and LM Studio runtime calls exist with endpoint allowlist enforcement, redirect blocking, bounded request and response payload validation, bounded retry/backoff and in-process circuit breakers for retry-exhausted generation failures, normalized usage/cost metadata, safe provider telemetry, and NDJSON streaming; a disabled external placeholder and an OpenAI-compatible external adapter exist, with external non-streaming and streaming generation disabled until configured with an HTTPS base URL, model allowlist, credential env-var reference, and explicit approval; configured external generation uses single-use bound provider approval IDs outside development/test mode, defers API-key/header resolution until pricing/config/circuit/approval gates allow transport, can calculate advisory usage-based cost estimates from an exact provider/model pricing catalog, and can pin roles to exact eligible provider/model routes. Encrypted credential storage, durable multi-worker circuit state, provider-specific billing reconciliation, and provider-specific external adapters remain.
