...
Status: backend MVP sprint surface active; Sprint 15 production identity, secrets, and network guardrails is in progress, with BL-009a through BL-009t implemented.
...
- Full production identity management beyond operator groups, managed KMS integration, first-class external secret-manager adapters, web retrieval network enforcement, and OS-level egress isolation: planned for remaining Sprint 15 follow-up; persisted operator profiles, persisted operator groups with capability inheritance, generated token lifecycle APIs, external credential references, encrypted local credential-vault references with supplied-key rotation, shell-free external-process credential resolver adapters, provider-call network/domain guardrails with bound approval records, generated-tool Python socket network policy enforcement, task-scoped active orchestration context checks, method-aware CLI approval reviewer capability separation, explicit CLI executable path boundary enforcement, top-level shell startup hardening, bare executable workspace/PATH trust checks, configured-safe command path-argument hardening, nested shell startup hardening checks, and identity/token/credential metadata redaction are implemented as Sprint 15 slices.
