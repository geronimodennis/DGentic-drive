...
- Ollama, LM Studio, and disabled-by-default OpenAI-compatible external provider contracts, with health/model probes for local runtimes, local generation calls, NDJSON streaming for OpenAI-compatible runtimes, provider-scoped egress allowlist checks with redirect blocking, bounded retry/backoff for retryable generation failures, safe provider response metadata, HTTPS-only credential-safe external adapter configuration, single-use bound external provider approvals, and scored provider routing.
...
- Provider system: Ollama and LM Studio runtime calls exist with endpoint allowlist enforcement, redirect blocking, bounded request and response payload validation, bounded retry/backoff for retryable generation failures, safe provider telemetry, and NDJSON streaming; a disabled external placeholder and an OpenAI-compatible external adapter exist, with external non-streaming and streaming generation disabled until configured with an HTTPS base URL, model allowlist, credential env-var reference, and explicit approval; configured external generation uses single-use bound provider approval IDs outside development/test mode. Encrypted credential storage, circuit-breaker/cost handling, and provider-specific external adapters remain.
