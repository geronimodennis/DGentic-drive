...
- Memory and retrieval: memory record storage, text/tag search, SQLAlchemy metadata index services, metadata CRUD routes, deterministic semantic retrieval fallback, SQLite vector backend abstraction, stored vector retrieval, lifecycle metadata, preview/apply lifecycle APIs, deterministic metadata-description compression, and retrieval API tests exist; pgvector integration, scheduled lifecycle/compression jobs, full-content or LLM summarization, broader performance validation, and source-attribution/scoring improvements remain production follow-up work.
