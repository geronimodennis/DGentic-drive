...
- Backend orchestration runs for multi-agent task graphs, including dependency-aware scheduling into sub-agent briefs, machine-readable role-boundary decisions, blocker and follow-up records, retry escalation, persisted run history, and Definition of Done evidence gates before closeout.
...
- Agent orchestration: lifecycle contracts, documentation, backend-managed task graphs, machine-readable role-boundary decisions with canonical declared-path validation, dependency scheduling, retry escalation, blockers/follow-ups, bounded scheduling passes, actor attribution when auth is enabled, and DoD close gates exist; a real autonomous execution loop, automatic backlog/progress mutation from run events, shared context/memory coordination, runtime binding between orchestration roles and filesystem/CLI/tool actions, production multi-agent scheduling, and UI/operations surfacing remain.
