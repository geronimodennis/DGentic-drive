...
- Provider system: Ollama and LM Studio runtime calls exist with endpoint allowlist enforcement, redirect blocking, bounded retry/backoff for retryable generation failures, safe provider telemetry, and NDJSON streaming; a disabled external placeholder and an OpenAI-compatible external adapter exist, with external non-streaming and streaming generation disabled until configured with an HTTPS base URL, model allowlist, credential env-var reference, and explicit approval; configured external generation uses single-use bound provider approval IDs outside development/test mode. Encrypted credential storage, circuit-breaker/cost handling, broader provider payload validation, and provider-specific external adapters remain.
