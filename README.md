...
Status: backend MVP sprint surface active; Sprint 15 production identity, secrets, and network guardrails is in progress, with BL-009a through BL-009ab implemented.
...
- Git workflow safety checkpoints with a shell-free, read-only `POST /cli/git/checkpoints` endpoint for commit/push/PR readiness metadata, root-bounded git inspection, checkpoint digests, protected branch/file and secret-shaped staged-addition blockers, test-evidence requirements, no-network PR warnings, authenticated requester binding, and command-policy protection so configured-safe `git` rules cannot downgrade mutating git commands.
