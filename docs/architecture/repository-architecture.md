...
- `GET /ui/`: Serves the static Sprint 16 dashboard shell, including the first task-chat planner with capped browser-local history, actionable task plan/run UI, session Git diff review decisions, editable bound execution panels, and responsive layout guards. The shell itself is public, but all protected API calls it makes still require an entered bearer token when auth is enabled.
