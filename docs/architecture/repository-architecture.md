...
- `web_retrieval.py`: Transport-free web retrieval network guard contract that pins future retrieval requests to the `web_retrieval` surface and `fetch` action, reuses domain policy, hook-policy escalation, active-task context checks, and bound network approval claiming, and intentionally avoids crawling, HTML parsing, or outbound fetch execution.
...
- `managed_policy_locks` only takes effect from `DGENTIC_MANAGED_SETTINGS_FILE`, not from ordinary environment variables, and can lock `cli_policy`, `command_recipes`, `hook_policy`, `plugin_trust`, `plugin_command_recipes`, and `plugin_hook_policies` mutation routes. The same managed file can declare `managed_cli_policy_rules` and `managed_hook_policy_rules`, which are parsed as deployment-owned read-only policy records, source-attributed as `managed`, evaluated before local rules, hidden from local mutable state persistence, and rejected by normal PATCH routes for their policy surfaces.
...
- `POST /web-retrieval/network/check`, `POST /web-retrieval/network/approvals`, and `POST /web-retrieval/network/authorize`: Evaluate, queue, and claim bound network approvals for future web retrieval fetches using the pinned `web_retrieval` surface and `fetch` action. These routes do not fetch remote content; they provide sanitized, approval-bound preflight contracts for future retrieval clients and require the `network` capability when auth is enabled.
...
- `POST /plugins/{plugin_id}/components/preview`: Reads bounded inert agent blueprint, skill, tool, and docs component references from a trusted current plugin manifest, returns type, name, component path, component digest, size, and manifest digest metadata, and does not parse, import, index, install, persist, load, or execute referenced content.
