...
- `network_policy.py`: Configurable outbound domain policy parser and evaluator with exact-domain and wildcard-subdomain rules plus `allow`, `deny`, `approval_required`, and `audit` modes.
- `provider_policy.py`: Shared provider endpoint policy, exact base URL normalization, network/domain policy enforcement, allowlist validation, and redirect-blocking HTTP opener.
