...
- `GET /ui/`: Serves the static Sprint 16 dashboard shell, including the first task-chat planner, actionable task plan/run UI, and session Git diff review decisions. The shell itself is public, but all protected API calls it makes still require an entered bearer token when auth is enabled.
