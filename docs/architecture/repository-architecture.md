...
- `ui/`: Static Sprint 16 dashboard shell served by FastAPI. It provides browser-session bearer-token handling, health cards, task planning, current-root workspace file browsing and text editing through guarded filesystem APIs, orchestration summary, unified approval inbox with approved CLI execution, CLI run output polling, Git checkpoint visibility, provider/tool summary, read-only CLI policy, command recipe, hook policy, and plugin trust visibility, effective settings, and event log polling while relying on the existing backend API capability gates for protected data and actions.
...
Recent web UI tests cover `/ui/` static entrypoint and asset serving, workspace file browser/editor endpoint wiring, approved CLI execution and CLI run output endpoint wiring, read-only policy/plugin endpoint wiring, session bearer-token header wiring, absence of client-side rootDir switching, plus the auth-enabled boundary where the dashboard shell remains loadable while protected API routes still require a bearer token.
