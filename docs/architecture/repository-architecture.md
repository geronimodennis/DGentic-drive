...
- `ui/`: Static Sprint 16 dashboard shell served by FastAPI. It provides browser-session bearer-token handling, health cards, task planning, active root context visibility, project root preflight and registration controls, current-root workspace file browsing and text editing through guarded filesystem APIs, orchestration summary, unified approval inbox with approved CLI execution, CLI run output polling, structured Git checkpoint blockers/warnings/diff-stat review, provider/tool summary, read-only CLI policy, command recipe, hook policy, and plugin trust visibility, effective settings, and event log polling while relying on the existing backend API capability gates for protected data and actions.
...
Recent web UI tests cover `/ui/` static entrypoint and asset serving, active root context display wiring, project registry/preflight endpoint wiring without activation, workspace file browser/editor endpoint wiring, approved CLI execution and CLI run output endpoint wiring, structured Git checkpoint renderer wiring, read-only policy/plugin endpoint wiring, session bearer-token header wiring, plus the auth-enabled boundary where the dashboard shell remains loadable while protected API routes still require a bearer token. Project registry tests cover canonical root validation, duplicate root rejection, registry persistence, admin-only route capability, active-root matching, and the guarantee that registration does not switch `/settings/effective` or guarded filesystem list behavior.
