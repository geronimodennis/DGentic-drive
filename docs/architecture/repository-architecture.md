...
- `POST /cli/execute`: Executes allowed commands or approval-required commands with a bound `approval_id` inside `rootDir` after cwd, read-only operand, and explicit executable path boundary checks, with audited agent/task context, orchestration-bound active task checks when context is supplied, and controlled environment overrides. The `approved: true` bypass is limited to development/test mode.
- `POST /cli/runs`: Starts an allowed asynchronous command run or an approval-required run with a bound `approval_id`, audited agent/task context, orchestration-bound active task checks when context is supplied, and controlled environment overrides. The `approved: true` bypass is limited to development/test mode.
