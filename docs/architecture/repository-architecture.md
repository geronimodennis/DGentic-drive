...
- `orchestration.py`: Backend orchestration control plane for persisted task graphs, canonical declared-path role-boundary validation, dependency-aware sub-agent scheduling, orchestration-bound filesystem and CLI action authorization, retry escalation, blockers, follow-ups, bounded scheduling passes, actor-attributed events when auth is enabled, closed-run immutability, and DoD evidence close gates.
