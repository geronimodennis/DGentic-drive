...
- `managed_policy_locks` only takes effect from `DGENTIC_MANAGED_SETTINGS_FILE`, not from ordinary environment variables, and can lock `cli_policy`, `command_recipes`, `hook_policy`, `plugin_trust`, `plugin_components`, `plugin_command_recipes`, and `plugin_hook_policies` mutation routes. The same managed file can declare `managed_cli_policy_rules`, `managed_hook_policy_rules`, and `managed_plugin_trust_records`; these are parsed as deployment-owned read-only policy records, source-attributed as `managed`, evaluated before local records, hidden from local mutable state persistence, and rejected by normal mutation routes for their policy surfaces.
