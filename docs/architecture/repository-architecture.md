...
- `orchestration.py`: Backend orchestration control plane for persisted task graphs, canonical declared-path role-boundary validation, dependency-aware sub-agent scheduling, orchestration-bound filesystem, CLI, and generated-tool action authorization, agent lifecycle reconciliation cycles, retry escalation, blocked-task recovery, blockers, follow-ups, bounded scheduling passes, actor-attributed events when auth is enabled, closed-run immutability, and DoD evidence close gates.
...
- `POST /tasks/orchestrations/{run_id}/advance`: Attempts to schedule pending tasks whose dependencies are completed.
- `POST /tasks/orchestrations/{run_id}/cycle`: Reconciles terminal spawned-agent lifecycle states back into running orchestration tasks, applies retry/block behavior, and schedules newly dependency-ready tasks.
- `PATCH /tasks/orchestrations/{run_id}/tasks/{task_id}`: Updates a running task to completed, failed, or blocked, then applies retry, blocker, follow-up, and dependency scheduling behavior.
