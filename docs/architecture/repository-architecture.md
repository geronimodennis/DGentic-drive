...
- Let deployment-owned managed settings coarse-lock mutable policy surfaces and publish read-only managed CLI policy rules. Surface locks fail closed for mutation routes while preserving read, preview, and evaluation access for operators and UI clients; managed CLI rules are evaluated before local rules but still cannot downgrade hard-coded command safety boundaries.
