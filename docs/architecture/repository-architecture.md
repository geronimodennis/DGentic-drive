...
- Execute CLI commands only through configurable command policy evaluation, root-bound and cwd-aware working directories, orchestration-bound active task checks when agent/task context matches a running orchestration task, controlled inherited environments plus explicit non-sensitive overrides, single-use approval records for approval-required commands outside development/test mode, sanitized output capture, persisted run history, and audit logging.
