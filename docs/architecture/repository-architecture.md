...
Recent Sprint 15 managed-policy tests also cover deployment-managed plugin component records, including managed-only settings loading, read-only component API behavior, local spoof filtering, managed-over-local overlay, no `plugin-components.json` persistence for managed rows, and `stale`/`drifted` status reporting.
