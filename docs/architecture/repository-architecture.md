...
- `git_workflows.py`: Shell-free read-only git workflow checkpoint service for commit, push, and PR preparation. It resolves `git` outside `rootDir`, bounds cwd and repository roots under `rootDir`, inspects status/ahead-behind/diff stats without optional locks or prompts, returns readiness blockers and warnings, redacts paths and secret-shaped findings, records bounded audit metadata, and intentionally avoids `git add`, `git commit`, `git push`, `gh`, or network PR operations.
