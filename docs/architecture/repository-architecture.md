...
- `GET /ui/`: Serves the static Sprint 16 dashboard shell, including the first task-chat planner, actionable task plan/run UI, session Git diff review decisions, editable bound execution panels, and responsive layout guards. The shell itself is public, but all protected API calls it makes still require an entered bearer token when auth is enabled.
...
- The backend approval review contract, structured dashboard review summaries, and first editable non-CLI bound execution UX are implemented; richer type-specific editors and end-to-end approval scenarios remain BL-010/Sprint 16 work.
