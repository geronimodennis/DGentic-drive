...
- `api/routes.py`: HTTP routes for health checks, persisted auth-token management, tasks, orchestration runs and detached orchestration execution polling, guardrails, CLI policy and approvals, provider approvals, providers, routing, owner-scoped orchestration agent reads, memory, tools, sessions, and logs.
