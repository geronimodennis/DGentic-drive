...
- `command_recipes.py`: Persisted command recipe registry for safe, parameterized command templates. Recipes expand into existing CLI execution requests, reject secret-shaped text and unsafe parameter values, expose policy preview contracts, and record usage without introducing a recipe-specific runner or approval bypass.
...
- `plugins.py`: Backend-only plugin manifest discovery and trust records for direct `rootDir/plugins/[plugin_id]/dgentic-plugin.json` manifests. The service never imports, loads, or executes plugin content; it validates exact root-bound manifest paths, rejects symlinked/out-of-root/oversized/malformed manifests, computes SHA-256 digests over raw manifest bytes, returns redacted safe metadata summaries, persists explicit trust/block decisions in `plugin-trust.json`, and marks trust stale when manifest bytes change.
...
- Discover plugins only from direct `rootDir/plugins/[plugin_id]/dgentic-plugin.json` manifest files, compute trust digests from exact raw manifest bytes, and keep plugin package loading, installation, command recipes, hook policies, and execution out of the discovery/trust foundation.
