...
- `POST /cli/git/change-review-artifacts`: Saves a metadata-only AI-change review artifact for a fresh matching Git checkpoint digest. The request stores section decisions and validates section patch digests against the current raw diff review; it does not persist raw patch bodies.
