...
- `main.py`: FastAPI app factory, application instance, a same-origin static `/ui/` dashboard mount, and an event-loop-scoped in-process HTTP request barrier that keeps project-root activation from overlapping other dashboard/API request handling in the local runtime.
- `ui/`: Static Sprint 16 dashboard shell served by FastAPI. It provides browser-session bearer-token handling, health cards, first chat-style task planning through the existing `/tasks/plan` and `/tasks/execute` contracts with capped browser-local transcript history, task-chat execution transcript/status cards for deterministic run results, fresh task-chat plan to orchestration-run creation through the existing `/tasks/orchestrations` create contract, a task-chat context stream using existing project/task/orchestration/memory/session/approval/log reads plus reusable plan/run/orchestration/memory/session/log context and evidence controls, Task Chat handoff packet Markdown/JSON preview and copy controls, structured task planning, active root context visibility, project root preflight, registration, registered project metadata edit/archive/restore controls, and guarded Open controls, active-root workspace file browsing and text editing plus guarded editor change preview/apply/revert controls through the existing filesystem APIs, guided orchestration task graph creation that writes the existing JSON create contract, orchestration summary/detail with task graph, expandable per-task sub-agent briefs and parent-child agent graph visibility from the existing `/agents` contracts when authorized, task update/recovery/blocker resolution/closeout controls backed by existing orchestration mutation endpoints, blockers, follow-ups, execution records, and cycle/loop/background execution controls, unified approval inbox source/status filtering with structured safe review summaries, warning/binding/digest/audit context, approved CLI execution, and backend contract coverage for all five approval source routes, CLI run output polling, command recipe preview/approval/run/execute controls plus local command recipe creation/edit/toggle controls through existing recipe contracts, trusted-plugin activation controls for inert reference components and declarative command recipe/hook-policy components through existing plugin activation contracts, structured Git checkpoint blockers/warnings/diff-stat review plus AI-change metadata summaries, checkpoint-bound raw Git diff review with session accept/reject annotations, bounded per-section reviewer rationale notes, decision filtering, bulk visible section decisions, and copyable patch/review evidence, checkpoint-bound commit/push/PR approval creation, and direct checkpoint-bound commit/push/PR run controls through existing Git workflow routes, provider/tool summary, Task Chat provider message-role selection for guarded provider replies and provider approval requests, bounded Task Chat approval outcome cards for decisions and executions, Activity session summary capture/list/use-context controls through existing session APIs, session log filtering, Activity log context/evidence controls, memory lifecycle and tool-registry reliability summaries with metadata/tool detail drilldowns, memory detail and retrieval result Use In Task Chat controls, lifecycle policy threshold controls, and guarded memory metadata quick-edit controls for editable metadata records, read-only CLI policy, hook policy, plugin trust, and policy source/status summaries, local CLI and hook policy rule creation/edit/toggle controls through existing guarded policy routes, grouped effective settings with managed-field, policy-lock, and provider-role-routing summaries, and event log polling while relying on the existing backend API capability gates for protected data and actions.
  The Policy panel also creates, lists, edits, and toggles local network-domain policy rules through guarded `/network/policy/rules` routes while rendering managed network rules and locked `network_policy` surfaces as read-only.
  The task planner portion now renders actionable task plan cards with step detail, context chips, related deterministic task-run history, a Run Plan action that posts the existing `TaskPlan` contract to `/tasks/execute`, task-chat execution cards that summarize deterministic run status, a fresh-plan Create Orchestration action that posts only to `/tasks/orchestrations` without starting orchestration loops or detached execution records, orchestration context insertion controls for created or recent runs, and pending-approval context cards that open the exact safe approval review in the unified inbox.
  The Task Chat provider portion can ask selected providers through the existing guarded provider generation and streaming routes, create pending provider approval requests through the existing provider approval route while omitting execution-only approval ids, choose the supported provider message role used by both flows, render provider replies and provider approval request cards in the transcript, fill the Ask Provider approval id from a created approval, and open the exact provider review in the unified inbox.
  The Task Chat route-preview portion calls the existing `/routing/decide` contract, renders bounded Provider Route cards, applies the selected provider/model back into the chat provider controls, and inserts route context without starting generation or creating approvals.
  The Task Chat memory portion lists active SQL memory metadata in the context stream and lets memory detail or hybrid retrieval rows insert bounded metadata/retrieval context into the composer without new memory mutation authority.
  The Task Chat handoff-packet portion assembles a bounded client-side continuation packet from composer, active-root, recent context, Activity/session/log, approval, provider route/reply, and transcript state, then copies Markdown or JSON without persisting backend packet state.
  The Task Chat approval-outcome portion appends bounded outcome cards after approval decisions, CLI approval execution, or dashboard-callable bound execution, then lets operators reuse the outcome as context or reopen the exact safe review without adding chat-side decision authority.
  The approval review portion shows editable bound execution panels for approved non-CLI approval families, with existing endpoint targets, safe payload scaffolds, recursive guided field controls that sync nested scalar edits into the canonical JSON payload, binding checks, direct execution for dashboard-callable bound requests, and handoff-only payloads for provider/tool network approvals instead of bypassing filesystem, network, provider, or tool request-binding checks.
...
- `schemas.py`: Pydantic contracts for tasks, execution runs, orchestration runs, detached orchestration execution records, explicit orchestration shared-memory tags and reuse policy, guardrails, hook policy rules and decisions, network policy decisions and editable network policy rules, CLI policy rules, command context, controlled command environments, providers, routing, agents, memory, tools, sessions, logs, auth/operator audit events, and credential audit events.
...
Recent web UI tests cover `/ui/` static entrypoint and asset serving, favicon browser-noise suppression, responsive panel shrink/wrap CSS guards, task-chat selector/helper wiring, task-chat local history helpers, task-chat context stream wiring, task-chat handoff packet preview/copy wiring, task-chat plan/run/orchestration/memory/session/log follow-up context controls, task-chat execution transcript/status card wiring, browser-driven task-chat plan/run/evidence insertion behavior, browser-driven fresh task-chat plan to orchestration-run creation and context insertion without detached execution records, browser-driven task-chat memory context insertion from active-memory cards, detail rows, and hybrid retrieval rows, browser-driven Activity session summary capture/list/use-context behavior, session log filtering, Activity log context handoff, and Task Chat handoff packet Markdown/JSON copying, task plan/execute endpoint reuse, active root context display wiring, project registry/preflight/activation/update endpoint wiring, browser-driven project metadata edit/archive/restore behavior, workspace file browser/editor endpoint wiring plus browser-driven workspace apply/revert behavior, orchestration creation form and POST payload wiring, guided task graph builder wiring, orchestration detail/action-control wiring, per-task agent-brief and agent-graph wiring, orchestration task update/recovery/blocker resolution/closeout endpoint wiring, approval source/status filter wiring, structured approval review summary and warning/audit wiring, guided non-CLI bound request field wiring, approved CLI execution and CLI run output endpoint wiring, browser-driven seeded CLI approval filter/review/approve behavior, browser-driven seeded filesystem approval review/approve/bound-execute behavior, browser-driven filesystem preflight-to-approval options and bound validation behavior, browser-driven seeded web-retrieval network approval review/approve/bound-fetch behavior, browser-driven seeded provider approval review/approve/bound-generate behavior, browser-driven seeded generated-tool approval review/approve/bound-execute behavior, provider browser network-approval consumption, generated-tool browser network-approval consumption, command recipe action endpoint wiring, local command recipe creation/edit/toggle form wiring with managed lock handling, structured Git checkpoint and AI-change metadata renderer wiring, checkpoint-bound raw Git diff review wiring, session Git diff accept/reject decision controls, decision filtering, bulk visible decisions, per-section reviewer rationale notes, evidence/patch-copy wiring, and matching-checkpoint review artifact restore, checkpoint-bound Git approval form and endpoint wiring, direct checkpoint-bound Git run button/endpoint/result wiring, provider health/routing and generated-tool governance wiring, memory retrieval, memory lifecycle preview/apply with policy threshold controls, memory compression preview/apply, memory/tool reliability endpoint wiring, read-only memory/tool reliability detail drilldown wiring, filesystem guardrail preflight/approval-request wiring and browser smoke behavior, read-only policy/plugin endpoint wiring with policy review summaries, local CLI policy creation/edit/toggle form wiring, local hook policy creation/edit/toggle form wiring with managed lock handling, grouped effective-settings review and provider role-routing card wiring, session bearer-token header wiring, plus the auth-enabled boundary where the dashboard shell remains loadable while protected API routes still require a bearer token. Project registry and activation tests cover canonical root validation, duplicate root rejection, registry persistence, metadata update/archive state, admin-only route capability, active-root matching, the guarantee that registration alone does not switch `/settings/effective` or guarded filesystem list behavior, successful activation with anchored state, old-root file invisibility after switching, unexecuted approval blockers, active CLI run blockers, running orchestration task blockers, archived project rejection, and invalid/deleted root rejection.
...
- `GET /ui/`: Serves the static Sprint 16 dashboard shell, including the first task-chat planner with capped browser-local history, Task Chat provider replies through existing guarded provider routes with selectable message roles, Task Chat provider approval request handoff through the existing guarded provider approval route using the selected role, bounded Task Chat approval outcome handoff cards, actionable task plan/run UI, task-chat execution transcript/status cards, fresh task-chat plan to orchestration-run creation through `/tasks/orchestrations`, reusable plan/run/orchestration/memory/session/log/provider-response/approval-outcome context and evidence insertion controls, client-side Task Chat handoff packet Markdown/JSON preview and copy controls, Activity session summary capture/list/use-context controls, Activity log context/evidence controls, session Git diff review decisions with reviewer rationale notes, editable guided bound execution panels with filesystem path/target/option validation, local policy/command-recipe/plugin-trust controls plus network policy preflight/approval request and filesystem guardrail preflight/approval request controls, provider health checks, routing preview, provider routing settings review, provider approval request creation, non-streaming and streaming provider generation with optional bound provider/network approval IDs, generated-tool governance controls, active-memory task-chat context cards, read-only hybrid memory retrieval, lifecycle preview/apply with policy threshold controls, compression preview/apply, guarded quick edits for non-service-authored memory metadata, approval source routing pinned to backend contracts, and responsive layout guards. The shell itself is public, but all protected API calls it makes still require an entered bearer token when auth is enabled.
  Task Chat route-preview cards reuse the same safe routing decision data as the Providers panel and remain a handoff surface only; provider generation and approval execution still use their existing guarded routes.
...
- `POST /network/policy/rules`, `GET /network/policy/rules`, and `PATCH /network/policy/rules/{rule_id}`: Create, list, edit, and enable/disable persisted local network-domain policy rules. Managed network rules are returned ahead of local records and cannot be patched through these routes; `managed_policy_locks` for `network_policy` reject local mutation while preserving reads and policy evaluation.
...
- `POST /cli/git/change-review-artifacts`: Saves a metadata-only AI-change review artifact for a fresh matching Git checkpoint digest. The request stores section decisions plus bounded secret-redacted reviewer rationale notes and validates section patch digests against the current raw diff review; it does not persist raw patch bodies.
- `GET /cli/git/change-review-artifacts`: Lists saved Git change review artifacts, optionally filtered by action or checkpoint digest.
- `GET /cli/git/change-review-artifacts/{artifact_id}`: Retrieves one saved Git change review artifact for dashboard restore/review history.
...
- The backend approval review contract, structured dashboard review summaries, editable guided non-CLI bound execution UX, provider and generated-tool approval request builders, unified approval source contract tests, backend bound execution scenario coverage, browser-driven seeded CLI approval review/approve smoke, browser-driven seeded filesystem approval review/approve/bound-execute smoke, browser-driven seeded web-retrieval network approval review/approve/bound-fetch smoke, browser-driven seeded provider approval review/approve/bound-generate smoke, browser-driven seeded generated-tool approval review/approve/bound-execute smoke, provider/tool approval request creation smokes, provider browser network-approval consumption, and generated-tool browser network-approval consumption are implemented; deeper nested type-specific editors remain BL-010/Sprint 16 work.
