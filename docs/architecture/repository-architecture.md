...
- `plugins.py`: Backend-only plugin manifest discovery, trust records, and declarative command recipe plus hook-policy activation for direct `rootDir/plugins/[plugin_id]/dgentic-plugin.json` manifests. The service never imports plugin code, runs plugin scripts, or loads plugin hooks/tools; it validates exact root-bound manifest paths, rejects symlinked/out-of-root/oversized/malformed manifests, computes SHA-256 digests over raw manifest bytes, returns redacted safe metadata summaries, persists explicit trust/block decisions in `plugin-trust.json`, marks trust stale when manifest bytes change, and installs only bounded JSON command recipe and hook-policy components from trusted current manifests with component digest provenance.
