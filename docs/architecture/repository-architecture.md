...
- `api/routes.py`: HTTP routes for health checks, tasks, orchestration runs and detached orchestration execution polling, guardrails, CLI policy and approvals, provider approvals, providers, routing, owner-scoped orchestration agent reads, memory, tools, sessions, and logs.
...
- `POST /tasks/orchestrations/{run_id}/advance`: Attempts to schedule pending tasks whose dependencies are completed, passing redacted completed-dependency output summaries into each spawned dependent agent brief.
- `POST /tasks/orchestrations/{run_id}/cycle`: Reconciles terminal spawned-agent lifecycle states back into running orchestration tasks, applies retry/block behavior, and schedules newly dependency-ready tasks with redacted dependency-output context.
...
- `PATCH /tasks/orchestrations/{run_id}/tasks/{task_id}`: Updates a running task to completed, failed, or blocked, then applies retry, blocker, follow-up, and dependency scheduling behavior. Completed task output is redacted and bounded before it is included in downstream spawned-agent context.
