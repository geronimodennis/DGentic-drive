...
- `git_workflows.py`: Shell-free git workflow checkpoint service for commit, push, and PR preparation plus checkpoint-bound commit, push, and PR approval request construction. It resolves `git` outside `rootDir`, bounds cwd and repository roots under `rootDir`, inspects status/ahead-behind/diff stats without optional locks or prompts, hashes staged and unstaged diff content plus upstream remote URL digests into checkpoint digests without returning raw diff text or remote URLs, returns readiness blockers and warnings, redacts paths and secret-shaped findings, records bounded audit metadata, validates non-secret single-line commit messages and bounded non-secret PR title/body/base fields, builds generated `git commit -m ...`, `git push`, or constrained `gh pr create ...` CLI approval requests only when fresh ready checkpoint digests match, rejects caller-supplied remote/branch/refspec/flag/command payloads, requires PR branches to already be pushed and current with upstream before approval creation, and validates workflow-bound approvals against fresh checkpoint state before CLI approval claim. It intentionally avoids `git add`, direct `git commit`/`git push` execution during approval creation, `gh` execution during approval creation, or network PR operations.
