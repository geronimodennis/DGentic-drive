...
- Execute Ollama, LM Studio, and configured OpenAI-compatible chat requests through provider runtime contracts using provider-scoped egress policy, redirect blocking, bounded retry/backoff for retryable generation failures before stream bytes begin, NDJSON downstream streaming for Ollama and OpenAI-compatible chunks, safe response metadata, HTTPS-only credential-safe outbound headers, single-use bound approvals for external generation outside development/test mode, model allowlist checks, and generic upstream failure details; encrypted credential storage, circuit breakers, cost accounting, and broader provider payload validation remain follow-up work.
