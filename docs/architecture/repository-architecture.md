...
- `git_workflows.py`: Shell-free git workflow checkpoint service for commit, push, and PR preparation plus checkpoint-bound commit approval request construction. It resolves `git` outside `rootDir`, bounds cwd and repository roots under `rootDir`, inspects status/ahead-behind/diff stats without optional locks or prompts, hashes staged and unstaged diff content into the checkpoint digest without returning raw diff text, returns readiness blockers and warnings, redacts paths and secret-shaped findings, records bounded audit metadata, validates non-secret single-line commit messages, and builds generated `git commit -m ...` CLI approval requests only when a fresh ready commit checkpoint digest matches. It intentionally avoids `git add`, direct `git commit` execution, `git push`, `gh`, or network PR operations.
- `command_recipes.py`: Persisted command recipe registry for safe, parameterized command templates. Recipes expand into existing CLI execution requests, reject secret-shaped text and unsafe parameter values, expose policy preview contracts, record usage without introducing a recipe-specific runner or approval bypass, and fail closed for plugin-owned recipes when plugin trust, manifest digest, component digest, or activation status drifts.
- `cli_runtime.py`: CLI approvals, single-use bound approval IDs, hook-policy-bound approval digests, root-bound synchronous and asynchronous command execution, POSIX translation for policy-approved `cmd /c` and `cmd.exe /c` wrappers, top-level `cmd /d` AutoRun suppression and PowerShell `-NoProfile -NonInteractive` launch hardening, bare executable workspace/PATH trust checks for direct and `cmd /c` inner commands before approval claim or subprocess launch, Windows default PATHEXT fallback candidates, failed launch run records for claimed synchronous approvals, chunked output polling, supervision metadata, auditable lifecycle states, stale-running reconciliation, process-local cancellation, controlled environment construction with startup/preload injection override blocking, agent/task context auditing, output redaction/truncation, and command run history.
...
- `hook_policy.py`: Persisted backend hook-policy rule storage for command, filesystem, and network guardrail surfaces. Rules support ordered matching, optional agent-role scoping, audit/approval/block effects, redacted persistence and audit metadata, and safe pre-action escalation without loading or executing plugin hook content.
- `network_policy.py`: Configurable outbound domain policy parser and evaluator with exact-domain and wildcard-subdomain rules plus `allow`, `deny`, `approval_required`, and `audit` modes, hook-policy escalation, backed by redacted single-use network approval records for approval-required provider requests and active orchestration task-context verification before approval create/validate/claim side effects.
...
- `providers.py`: Provider registry, policy-validated Ollama/LM Studio health and model probes, disabled external placeholder, config-only OpenAI-compatible external provider status through configured credential references or env-var fallback, external-process adapter configuration validation without secret retrieval, pricing-aware provider estimates, role-aware provider/model routing, and scored routing decisions.
- `provider_runtime.py`: Ollama, LM Studio, and OpenAI-compatible external chat/completion request execution with provider endpoint and network-domain policy enforcement, `network_approval_id` binding for approval-required provider domains, active orchestration task-context verification before approval creation, credential lookup, approval claim, or transport, Ollama/OpenAI-compatible streaming, bounded retry/backoff, in-process circuit breakers, deferred credential-safe headers for external fail-fast paths, credential-reference resolution at transport time, local encrypted vault and shell-free external-process credential adapter support, model allowlist checks, single-use bound external provider approvals, advisory usage-based cost estimates, and safe response telemetry.
- `credentials.py`: Persisted references to credential locations or ciphertext, including environment variables, operator-key encrypted local vault records, and configured shell-free external process adapters, with lifecycle/audit helpers, counts-only local vault key rotation, no raw secret-value response or event storage, external-process timeout/output bounds, and secret-shaped metadata redaction before responses, credential audit metadata, and new or mutated JSON persistence.
- `plugins.py`: Backend-only plugin manifest discovery, trust records, and declarative command recipe activation for direct `rootDir/plugins/[plugin_id]/dgentic-plugin.json` manifests. The service never imports plugin code, runs plugin scripts, or loads plugin hooks/tools; it validates exact root-bound manifest paths, rejects symlinked/out-of-root/oversized/malformed manifests, computes SHA-256 digests over raw manifest bytes, returns redacted safe metadata summaries, persists explicit trust/block decisions in `plugin-trust.json`, marks trust stale when manifest bytes change, and installs only bounded JSON command recipe components from trusted current manifests with component digest provenance.
...
- `managed_policy_locks` only takes effect from `DGENTIC_MANAGED_SETTINGS_FILE`, not from ordinary environment variables, and can lock `cli_policy`, `command_recipes`, `hook_policy`, `plugin_trust`, and `plugin_command_recipes` mutation routes.
...
- `GET /guardrails/hooks/rules`: Lists hook policy rules in evaluation order.
- `PATCH /guardrails/hooks/rules/{rule_id}`: Updates a hook policy rule without storing secret-shaped match patterns.
...
- `POST /credentials/references`: Creates a persisted credential reference backed by an environment variable, a configured external process adapter, or a local encrypted vault ciphertext. Local vault creation accepts a raw `secret_value` only in the create request, encrypts it with the operator-provided `DGENTIC_CREDENTIAL_VAULT_KEY`, and omits plaintext plus ciphertext from API views and credential audit events.
- `GET /credentials/references`: Lists persisted credential references.
- `POST /credentials/references/{credential_ref_id}/revoke`: Revokes a persisted credential reference so provider runtime resolution fails closed.
...
- `POST /tools/{name}/execute`: Executes a registered generated tool, blocks deprecated/disabled/blocked tools, fails closed on SQL registry permission conflicts, requires a single-use bound `approval_id` for approval-required tools outside development/test mode, enforces orchestration-bound active task checks when agent context is supplied, launches Python with isolated import semantics and only tool-local dependency paths, passes only sanitized network-domain policy decisions into the subprocess runner, installs common Python socket egress guards before generated tool imports run, starts the subprocess in a process group or new process group where the host supports it, cleans up the process tree on timeout, uses a reduced inherited environment, redacts stdout/stderr/parsed output for common secret patterns, records execution audit metadata with serialized orchestration decisions, syncs SQL registry usage counters when a row exists, and applies reliability policy warnings/disable/deprecation after enough runtime evidence.
...
- `GET /plugins`: Discovers direct plugin manifests under `rootDir/plugins/[plugin_id]/dgentic-plugin.json`, returns redacted metadata summaries and safe discovery errors, and rejects symlinked, out-of-root, oversized, or malformed manifests without loading plugin content.
- `GET /plugins/{plugin_id}`: Returns one discovered plugin summary by id.
- `PATCH /plugins/{plugin_id}/trust`: Persists an explicit `trusted` or `blocked` decision for the current manifest digest, with redacted reason and actor metadata. Later manifest byte changes surface the trust status as `stale`.
...
- Discover plugins only from direct `rootDir/plugins/[plugin_id]/dgentic-plugin.json` manifest files, compute trust digests from exact raw manifest bytes, and keep plugin package loading, hooks, tools, agents, skills, scripts, and arbitrary execution out of the discovery/trust foundation.
- Allow trusted current plugin manifests to activate declarative command recipe JSON components only through digest-bound provenance records and the existing command recipe/CLI runtime gates; component drift, stale trust, blocked trust, or disabled activation must fail closed before preview or execution.
...
- Let deployment-owned managed settings coarse-lock mutable policy surfaces before richer per-record policy-source precedence exists. Surface locks fail closed for mutation routes while preserving read, preview, and evaluation access for operators and UI clients.
