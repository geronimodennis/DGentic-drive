...
- `provider_runtime.py`: Ollama and LM Studio chat/completion request execution with provider endpoint policy enforcement, bounded retry/backoff, and safe response telemetry.
...
- Execute Ollama and LM Studio chat requests through provider runtime contracts using the shared provider egress policy, redirect blocking, bounded retry/backoff for retryable generation failures, safe response metadata, and generic upstream failure details; streaming, circuit breakers, and external providers remain follow-up work.
