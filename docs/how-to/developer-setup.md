...
Local network-domain policy rules can be managed through `POST /network/policy/rules`, `GET /network/policy/rules`, and `PATCH /network/policy/rules/{rule_id}` or through the dashboard Policy panel. Managed rules remain read-only and evaluate first, legacy `network_domain_policy.rules` stay compatible, and local persisted rules evaluate after managed and legacy rules. Add `"network_policy"` to `managed_policy_locks` when deployment-managed network policy should block local create/update while preserving reads and preflight evaluation.
...
The `/ui/` dashboard is served by the same FastAPI process. In local development it can call the API without a token unless auth is explicitly enabled. In staging or production, open the dashboard, enter a bearer token with the needed capabilities, and use the UI surfaces for task planning, Task Chat provider prompt preview before generation, Task Chat provider replies with selectable message roles and response-to-reply handoff, Task Chat provider approval request creation/review handoff using the selected message role, Task Chat approval outcome cards after decisions or executions with explicit outcome-to-reply handoff, task-chat context cards, Task Chat handoff packet Markdown/JSON preview and copy controls, Task Chat composer context review/redact/clear and per-block remove/order controls, plan/run/memory/session/log/provider-response/approval-outcome follow-up context controls, active root context visibility, project root preflight, registration, metadata edit/archive/restore controls for registered projects, and guarded Open controls, active-root workspace file browsing/editing with guarded change preview/apply/revert controls, guided orchestration task graph creation with raw JSON fallback, orchestration summary/detail with task graph, expandable per-task sub-agent briefs, parent-child agent graph visibility, task update/recovery/blocker resolution/closeout controls, and execution controls, approval source/status filtering and review/decisions, approved CLI execution, recursive guided non-CLI bound payload editing with filesystem path/target/option validation, CLI run output inspection, command recipe preview/approval/run/execute controls plus local command recipe creation/edit/toggle controls, local plugin trust/block controls and trusted-plugin activation controls, network policy preflight checks and approval requests, filesystem guardrail preflight checks and approval requests with action-specific options/content fields, structured Git checkpoint review, checkpoint-bound raw diff review with decision filters, bulk visible decisions, per-section patch copy, reviewer rationale notes, and saved metadata-only review artifacts, provider/tool summaries, provider health checks, provider routing preview, provider routing settings review, provider approval request creation, provider generation and streaming execution with optional bound provider/network approval IDs and task-chat response context insertion, generated-tool governance and approval request controls, memory lifecycle summaries, active-memory task-chat context cards, retrieval with Use In Task Chat controls, Activity session summary capture/list/use-context controls, Activity log Use Context/Copy Evidence controls, session log filtering, lifecycle preview/apply with policy threshold controls, compression preview/apply, tool-registry reliability summaries, memory/tool detail drilldowns, guarded quick edits for non-service-authored memory metadata, CLI policy, hook policy, and plugin trust visibility, local CLI, hook policy, and network policy rule creation/edit/toggle controls, effective settings with provider role-routing cards, and logs. Project Open switches the active runtime `DGENTIC_ROOT_DIR` inside the current FastAPI process only after blocking active CLI/orchestration work and unexecuted approvals; relative `DGENTIC_DATA_DIR` is pinned to its current absolute location during the switch. Agent brief and graph detail use the existing `/agents` capability; without it, orchestration tasks still render and the agent detail areas fail softly. Project registration, metadata update/archive/restore, and blocker resolution remain admin-gated by the backend. Plugin trust and inert reference component activation use the existing `tools` capability plus managed `plugin_trust` and `plugin_components` read-only gates where applicable, plugin command recipe activation uses `tools` plus `cli`, plugin hook-policy activation uses `tools` plus `hooks`, tool detail drilldowns, generated-tool governance changes, and generated-tool approval request creation use the existing `tools` capability, memory retrieval, memory detail drilldowns, memory-to-task-chat context insertion, lifecycle preview/apply, compression preview/apply, and editable metadata quick edits use the existing `memory` capability, session summary capture/list flows use the existing `sessions` capability, log polling/filtering and Activity log context handoff use the existing `logs` capability, filesystem guardrail preflight, workspace file apply/revert, and filesystem approval requests use the existing `filesystem` capability, network policy preflight plus approval requests use the existing network/approval capability gates, provider health/routing/generation checks use the existing `providers` capability, and provider approval request creation uses the existing `approvals` capability.
...
The Task Chat Context Review panel shows composer context line, block, and byte counts. Preview Context renders a bounded redacted preview as display-only text and does not change the composer. The panel also lists blank-line-delimited context blocks with redacted titles/excerpts plus per-block Move Up, Move Down, and Remove controls. Stale rendered block controls refresh the review instead of mutating changed composer context. Redact Context explicitly replaces the full composer context with redacted text without trimming long content, and Clear Context empties the composer context before the next provider ask.
...
The Activity panel can capture session summaries through `/sessions/summary`, list recent summaries, filter `session` events through `/logs`, and add bounded summary context to Task Chat without editing backend session records beyond the existing create contract.
...
Task-chat provider prompt preview is local-only in the browser. The Preview Prompt action composes the same provider prompt used by Ask Provider, appends a bounded Provider Prompt Preview card with secret-shaped values redacted, and does not call provider generation, streaming generation, or approval routes.

Task-chat Provider Reply and Provider Stream cards include Use Response and Use Response & Ask actions. Use Response inserts bounded redacted provider output as follow-up context, while Use Response & Ask inserts the same context and then calls the existing guarded Task Chat provider reply path. This remains an explicit user action and does not create approvals, copy approval IDs, or add provider execution authority.

Task-chat plan, run, orchestration-run, session-summary, active-memory, and log context cards include Use Context & Ask actions. These actions insert bounded redacted context into the composer and then call the existing guarded Task Chat provider reply path; pending approval context cards remain review/context handoffs only and do not grant approval authority.
...
Task-chat approval outcome cards appear after approval decisions, approved CLI execution, or dashboard-callable bound execution. They include bounded source/status/reason metadata, a Use Outcome action for follow-up context, a Use Outcome & Ask action that inserts the same bounded context before invoking the existing provider reply path, and a Review action that reopens the exact safe review. Outcome context uses non-capability approval references, and Use Outcome & Ask does not expose raw outcome IDs or copy them into provider approval fields.
...
The Policy panel also exposes trusted-plugin activation controls. Use a bearer token with `tools` capability to preview/list/install/disable inert reference components, add `cli` for plugin command recipe activation, and add `hooks` for plugin hook-policy activation. Managed `plugin_components`, `plugin_command_recipes`, and `plugin_hook_policies` locks render the matching mutation controls read-only.
...
Discovery reads only the exact manifest bytes, computes a SHA-256 digest, returns redacted safe metadata, and stores trust records in `plugin-trust.json`. Trust becomes `stale` when the manifest bytes change. Trusted current manifests can preview inert agent blueprint, skill, tool, and docs component references with root-bound bounded reads, component SHA-256 digests, and component sizes; install persists only the same inert metadata in `plugin-components.json`, and disable marks those records inactive. These routes do not parse, import, index, load, or execute referenced content. The `/ui/` Policy panel surfaces the same preview/list/install/disable component routes plus plugin command recipe and hook-policy activation routes; managed plugin activation locks keep matching mutation controls read-only. Symlinked plugin directories, symlinked manifests, out-of-root manifests, manifests over 64 KiB, malformed JSON, or manifests whose `plugin_id` does not match the containing directory are rejected with safe errors. In staging or production, reference component routes require `tools`, command recipe activation requires `tools` plus `cli`, and hook-policy activation requires `tools` plus `hooks`.
...
Preview and apply the SQL-backed memory lifecycle policy. Preview is read-only; apply can promote, archive, or soft-prune matching metadata. The dashboard exposes matching archive, soft-prune, promote, and compression-candidate threshold controls in the Reliability panel. The memory detail view can also PATCH editable metadata tags, category, description, relevance, and retention policy for non-service-authored records and insert bounded metadata context into Task Chat; orchestration shared-memory records stay read-only. Compression candidates are executed through the separate compression endpoints:
...
- Local JSON persistence and SQLite-compatible semantic memory prototypes exist with local SQLite backup/restore helpers, additive lifecycle metadata migrations, lifecycle preview/apply APIs, deterministic metadata compression APIs, a SQLite JSON-vector backend abstraction, retrieval attribution/score explanations, baseline retrieval performance smoke coverage, and dashboard reliability summaries with retrieval, detail drilldowns, lifecycle policy threshold controls, confirmed lifecycle/compression apply controls, and guarded editable metadata quick-edit controls, but no pgvector production backend, scheduled memory lifecycle/compression job, broader memory administration UI, or VS Code extension exists yet.
