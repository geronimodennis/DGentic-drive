...
Use `POST /cli/git/commit-approvals` when the commit should remain a pending CLI approval. Push and PR workflows remain approval-bound through `POST /cli/git/push-approvals` and `POST /cli/git/pr-approvals`; they do not perform `git push`, run `gh`, or create network PRs during approval creation.
