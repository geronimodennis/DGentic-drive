...
Push approvals remain available through `POST /cli/git/push-approvals` when the push should stay in the normal approval queue. PR workflows remain approval-bound through `POST /cli/git/pr-approvals`; PR approval creation does not run `gh` or create network PRs.
