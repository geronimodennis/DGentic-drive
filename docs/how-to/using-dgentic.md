...
- API
- `/ui/` dashboard for task planning, guided orchestration creation/review, approvals, workspace files, command recipes, Git checkpoints with AI-change metadata, policy/source summaries, grouped settings review, and reliability summaries
...
- The first same-origin dashboard is implemented, including read-only settings/policy review summaries and Git checkpoint AI-change metadata, but full chat, full AI-change diff review, editable settings/policy UX, broader browser QA, VS Code extension, and dedicated CLI client still need to be built.
