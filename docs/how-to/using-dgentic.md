...
- The first same-origin dashboard is implemented, including a first task-chat composer/transcript, actionable task plan/run cards, structured approval review summaries, editable non-CLI bound execution request panels, read-only settings/policy review summaries, local CLI policy rule creation/edit/toggle controls, Git checkpoint AI-change metadata, checkpoint-bound raw Git diff review with session accept/reject decisions, checkpoint-bound Git approval creation, and direct checkpoint-bound Git run controls, but deeper unified chat, persistent AI-change artifact apply/revert workflows, richer type-specific non-CLI editors, broader editable settings/policy UX, broader browser QA, VS Code extension, and dedicated CLI client still need to be built.
