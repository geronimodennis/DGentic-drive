...
- Ollama and LM Studio have policy-validated local health/model probes and chat generation calls with redirect blocking, bounded retry/backoff for retryable generation failures, safe telemetry, and NDJSON streaming through `/providers/generate/stream`.
