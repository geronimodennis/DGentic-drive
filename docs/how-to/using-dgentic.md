...
- Guardrails enforce text and binary reads/writes, directory listing, metadata, approval-gated delete/move/copy/rename, and single-use bound filesystem approval records inside `rootDir`; configurable persisted filesystem policy rules, interactive filesystem approval UI, deeper locked-file handling, and OS-level filesystem isolation remain follow-up work.
