...
- The first same-origin dashboard is implemented, including actionable task plan/run cards, structured approval review summaries, read-only settings/policy review summaries, local CLI policy rule creation/edit/toggle controls, Git checkpoint AI-change metadata, checkpoint-bound raw Git diff review, checkpoint-bound Git approval creation, and direct checkpoint-bound Git run controls, but full chat, accept/reject AI-change artifact workflows, broader editable settings/policy UX, broader browser QA, VS Code extension, and dedicated CLI client still need to be built.
