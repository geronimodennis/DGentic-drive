...
- The first same-origin dashboard is implemented, including a first task-chat composer/transcript with capped local history, insertable task-chat context cards, reusable plan/run/orchestration context and evidence controls, task-chat execution transcript/status cards, fresh task-chat plan to orchestration-run creation, actionable task plan/run cards, workspace file change preview/apply/revert controls, structured approval review summaries, recursive guided editable non-CLI bound execution request panels, read-only settings/policy review summaries, local CLI policy, hook policy, and command recipe creation/edit/toggle controls, Git checkpoint AI-change metadata, checkpoint-bound raw Git diff review with session accept/reject decisions, saved metadata-only review artifacts, checkpoint-bound Git approval creation, and direct checkpoint-bound Git run controls, but richer unified chat semantics beyond deterministic execution, explicit orchestration creation, and reusable orchestration context, deeper Git hunk/patch apply or revert workflows beyond guarded workspace editor mutations, broader editable settings/policy UX beyond CLI policy, hook policy, and command recipes, VS Code extension, and dedicated CLI client still need to be built.
