...
Approved non-CLI approval reviews now show bound execution request handoffs instead of a direct execute button. Filesystem, network, provider, and tool approvals require their normal execution endpoint plus the approved `approval_id` and matching request fields; the dashboard shows the endpoint and a safe payload scaffold, while full request editors remain follow-up work.
...
- The first same-origin dashboard is implemented, including a first task-chat composer/transcript, actionable task plan/run cards, structured approval review summaries, non-CLI bound execution request handoff panels, read-only settings/policy review summaries, local CLI policy rule creation/edit/toggle controls, Git checkpoint AI-change metadata, checkpoint-bound raw Git diff review with session accept/reject decisions, checkpoint-bound Git approval creation, and direct checkpoint-bound Git run controls, but deeper unified chat, persistent AI-change artifact apply/revert workflows, full non-CLI request editors/execution, broader editable settings/policy UX, broader browser QA, VS Code extension, and dedicated CLI client still need to be built.
