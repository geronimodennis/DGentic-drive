...
The dashboard provides health, task planning, active root context visibility, project root preflight, registration, and guarded Open controls, active-root workspace file browsing/editing, guided orchestration task graph creation with raw JSON fallback, orchestration summary and detail with task graph, expandable per-task sub-agent briefs, parent-child agent graph visibility, task update/recovery/blocker resolution/closeout controls, blockers, follow-ups, execution records, and cycle/loop/background execution controls, unified approval source/status filtering, review and approve/deny actions, approved CLI execution, CLI run output inspection, command recipe preview/approval/run/execute controls, structured Git workflow checkpoint blockers/warnings/diff-stat review, provider/tool summary, memory lifecycle and tool-registry reliability summaries, read-only CLI policy, hook policy, and plugin trust visibility, effective settings, and log polling. In development, auth is off unless explicitly enabled. In staging and production, enter a bearer token with `admin` capability before using project registration, Open controls, or admin-only blocker resolution. A token with `tasks` but not `agents` can still load orchestration task data, but the richer sub-agent brief and graph panels will show agent detail as unavailable.
...
- API
- `/ui/` dashboard for task planning, guided orchestration creation/review, approvals, workspace files, command recipes, Git checkpoints, policy visibility, and reliability summaries
...
- The first same-origin dashboard is implemented, but full chat, AI-change review, editable settings/policy UX, broader browser QA, VS Code extension, and dedicated CLI client still need to be built.
