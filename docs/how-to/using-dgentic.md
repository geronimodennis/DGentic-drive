...
The dashboard provides health, task planning, active root context visibility, project root preflight, registration, and guarded Open controls, active-root workspace file browsing/editing, orchestration summary and detail with task graph, blockers, follow-ups, execution records, and cycle/loop/background execution controls, unified approval source/status filtering, review and approve/deny actions, approved CLI execution, CLI run output inspection, structured Git workflow checkpoint blockers/warnings/diff-stat review, provider/tool summary, read-only CLI policy, command recipe, hook policy, and plugin trust visibility, effective settings, and log polling. In development, auth is off unless explicitly enabled. In staging and production, enter a bearer token with `admin` capability before using project registration or Open controls.
