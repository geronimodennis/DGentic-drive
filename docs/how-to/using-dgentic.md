...
- External providers: an OpenAI-compatible non-streaming and streaming adapter is available when explicitly configured with HTTPS, a model allowlist, an env-referenced credential, and development/test approval or a staging/production bound provider approval ID; exact provider/model pricing can provide advisory usage and routing estimates, while dedicated Google AI, DeepSeek, Anthropic, Copilot, or other adapters remain future work.
