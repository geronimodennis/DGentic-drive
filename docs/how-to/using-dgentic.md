...
- API
- `/ui/` dashboard for task-chat planning with capped local history, structured task planning, deterministic plan runs, guided orchestration creation/review, structured approval review summaries, guided bound non-CLI execution request handoffs, workspace files, command recipes, Git checkpoints with AI-change metadata, checkpoint-bound raw diff review, session decisions, saved metadata-only review artifacts, checkpoint-bound approval creation, direct checkpoint-bound Git runs, policy/source summaries, local CLI policy rule creation/edit/toggle controls, grouped settings review, and reliability summaries
...
- The first same-origin dashboard is implemented, including a first task-chat composer/transcript with capped local history, actionable task plan/run cards, structured approval review summaries, guided editable non-CLI bound execution request panels, read-only settings/policy review summaries, local CLI policy rule creation/edit/toggle controls, Git checkpoint AI-change metadata, checkpoint-bound raw Git diff review with session accept/reject decisions, saved metadata-only review artifacts, checkpoint-bound Git approval creation, and direct checkpoint-bound Git run controls, but full unified chat, actual AI-change file apply/revert workflows, deeper nested type-specific non-CLI editors, broader editable settings/policy UX, VS Code extension, and dedicated CLI client still need to be built.
