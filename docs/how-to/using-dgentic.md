...
The dashboard provides health, first chat-style task submission with capped browser-local transcript history, task-chat context cards for active root, task/run counts, pending approvals, recent activity, insertable plan/run/approval/log context, plan/run cards that can insert follow-up context or evidence back into the composer, task planning, active root context visibility, project root preflight, registration, and guarded Open controls, active-root workspace file browsing/editing, guided orchestration task graph creation with raw JSON fallback, orchestration summary and detail with task graph, expandable per-task sub-agent briefs, parent-child agent graph visibility, task update/recovery/blocker resolution/closeout controls, blockers, follow-ups, execution records, and cycle/loop/background execution controls, unified approval source/status filtering, structured source-specific approval review summaries, review and approve/deny actions, approved CLI execution, CLI run output inspection, command recipe preview/approval/run/execute controls plus local command recipe creation/edit/toggle controls, local plugin trust/block controls, network policy preflight checks and approval requests, filesystem guardrail preflight checks and approval requests, structured Git workflow checkpoint blockers/warnings/diff-stat review plus AI-change metadata summary, checkpoint-bound raw Git diff review with session accept/reject decisions, metadata-only saved review artifacts that can be applied only to matching fresh checkpoint reviews, checkpoint-bound commit/push/PR approval creation, and direct checkpoint-bound Git commit/push/PR run controls, provider/tool summary with per-provider health checks, routing preview, and generated-tool governance controls, memory lifecycle summaries, read-only hybrid memory retrieval, lifecycle preview/apply, compression preview/apply, tool-registry reliability summaries, read-only memory/tool detail drilldowns, CLI policy, hook policy, plugin trust, policy source/status summaries, local CLI and hook policy rule creation/edit/toggle controls, grouped effective settings with managed-field and policy-lock summaries, and log polling. In development, auth is off unless explicitly enabled. In staging and production, enter a bearer token with `admin` capability before using project registration, Open controls, or settings review; use a token with `cli` capability for Git checkpoint diff review, Git review artifact save/load, Git checkpoint approval creation, direct Git workflow runs, local CLI policy rule creation or updates, and local command recipe creation or updates; use a token with `filesystem` capability for filesystem guardrail preflight, filesystem approval requests, and guarded filesystem execution; use a token with `memory` capability for memory retrieval, memory detail drilldowns, lifecycle preview/apply, and compression preview/apply; use a token with `hooks` capability for local hook policy rule creation or updates; use a token with `tools` capability for local plugin trust changes, tool detail drilldowns, and generated-tool governance updates; use a token with `network` capability for network policy preflight checks and web-retrieval approval requests, `providers` capability for provider health checks and routing preview, or the relevant approval capability for generic network approval creation. A token with `tasks` but not `agents` can still load orchestration task data, but the richer sub-agent brief and graph panels will show agent detail as unavailable.

The task area now includes a first task-chat composer that submits a message to the existing `/tasks/plan` contract, renders the created plan in a transcript, and can run that plan through `/tasks/execute`. Plan cards expose a `Use Context` action and run rows expose a `Use Evidence` action so follow-up turns can include the exact plan or deterministic run summary without retyping it. The transcript is saved in capped browser-local history and restored after reloads; the Clear control removes saved history. Restored plan cards are display-only so stale saved plans are not directly re-run, and bearer tokens remain session-only. The structured planner still renders recent plans as actionable cards with constraints, acceptance criteria, step detail, related deterministic run history, and a Run Plan control.
...
- Guardrails enforce text and binary reads/writes, directory listing, metadata, approval-gated delete/move/copy/rename, and single-use bound filesystem approval records inside `rootDir`; configurable persisted filesystem policy rules, deeper filesystem approval detail editors beyond preflight creation, deeper locked-file handling, and OS-level filesystem isolation remain follow-up work.
...
- The first same-origin dashboard is implemented, including a first task-chat composer/transcript with capped local history, insertable task-chat context cards, reusable plan/run context and evidence controls, actionable task plan/run cards, structured approval review summaries, recursive guided editable non-CLI bound execution request panels, read-only settings/policy review summaries, local CLI policy, hook policy, and command recipe creation/edit/toggle controls, Git checkpoint AI-change metadata, checkpoint-bound raw Git diff review with session accept/reject decisions, saved metadata-only review artifacts, checkpoint-bound Git approval creation, and direct checkpoint-bound Git run controls, but deeper full-chat execution semantics, actual AI-change file apply/revert workflows, broader editable settings/policy UX beyond CLI policy, hook policy, and command recipes, VS Code extension, and dedicated CLI client still need to be built.
