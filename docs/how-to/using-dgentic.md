...
- External providers: an OpenAI-compatible non-streaming and streaming adapter is available when explicitly configured with HTTPS, a model allowlist, a persisted external credential reference or env-var fallback, and development/test approval or a staging/production bound provider approval ID; exact provider/model pricing and role-specific provider/model routes can provide advisory usage and routing controls, credential value/header resolution is deferred on fail-fast paths, and dedicated Google AI, DeepSeek, Anthropic, Copilot, or other adapters remain future work.
