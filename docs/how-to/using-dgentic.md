...
Use `POST /cli/git/commit-approvals` instead when the commit should stay in the normal approval queue. Push and PR automation is still approval-bound through `POST /cli/git/push-approvals` and `POST /cli/git/pr-approvals`; those routes do not run `git push`, `gh`, or network PR operations during approval creation.
