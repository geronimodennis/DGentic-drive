...
Detached execution records are persisted with `starting`, `running`, `cancelling`, `cancelled`, `completed`, `failed`, or `stale` status. DGentic rejects a second active execution or foreground loop for the same orchestration run with a conflict response, keeps `cancelling` executions active until the owning worker finalizes them, renews a process-local heartbeat while a detached worker is running, marks old active records from other supervisors stale after a heartbeat timeout, and redacts failure errors before persistence. This is still a process-local daemon-thread worker: it does not resume/adopt a worker after backend restart, provide durable multi-worker leases, or replace the future production scheduler.
