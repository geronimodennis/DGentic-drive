...
Detached execution records are persisted with `starting`, `running`, `completed`, `failed`, or `stale` status. DGentic rejects a second active execution or foreground loop for the same orchestration run with a conflict response, renews a process-local heartbeat while a detached worker is running, marks old active records from other supervisors stale after a heartbeat timeout, and redacts failure errors before persistence. This is still a process-local daemon-thread worker: it does not cancel, resume/adopt a worker after backend restart, provide durable multi-worker leases, or replace the future production scheduler.
...
For opt-in shared memory across runs, set `shared_memory_tags` on the orchestration or an individual task. When a tagged task completes, DGentic upserts one SQL metadata record in category `orchestration_context` with a redacted, bounded task-output summary. Later tagged tasks receive up to three active matching summaries in their spawned agent brief context. DGentic only injects records produced by completed orchestration tasks, scoped to the same authenticated orchestration owner (or local `system` owner when auth is disabled); arbitrary metadata rows and inactive lifecycle records are ignored, and a consumer's tags must cover all tags on the stored shared-memory record.

```powershell
curl -X POST http://127.0.0.1:8000/tasks/orchestrations `
  -H "Content-Type: application/json" `
  -d '{"objective":"Reuse QA learnings.","shared_memory_tags":["qa-context"],"tasks":[{"id":"qa-validation","title":"QA validation","description":"Validate with shared memory.","role":"QA","declared_write_paths":["tests/test_api.py"],"validation":"QA receives shared memory."}]}'
```

You can filter SQL metadata records by tag through the metadata API:
...
- Backend orchestration runs can validate task graphs, enforce canonical declared role write boundaries, schedule dependency-ready tasks into sub-agent briefs with redacted dependency-output context, publish and reuse opt-in SQL-backed shared memory through explicit tags, reconcile terminal spawned-agent lifecycle statuses through explicit cycle and bounded loop endpoints, start a process-local detached bounded-loop execution with pollable persisted status, bind filesystem write actions to running task declared paths when agent context is supplied, bind CLI actions and generated-tool approvals/executions to exact running task context when active context is supplied, track blockers/follow-ups, retry failed tasks until the configured limit, recover role-boundary and retry-exhaustion blocked tasks after safe correction, resolve manual/security blockers with audit history, regenerate orchestration progress/follow-up documents, reject closed-run mutation, bound scheduling passes, and require DoD evidence before closeout, but shared-memory ACLs/policies beyond explicit tags, detached worker restart adoption/resume, cancellation, and production scheduling/lease semantics remain future work.
