...
- The first same-origin dashboard is implemented, including a first task-chat composer/transcript with capped local history, insertable task-chat context cards, actionable task plan/run cards, structured approval review summaries, recursive guided editable non-CLI bound execution request panels, read-only settings/policy review summaries, local CLI and hook policy rule creation/edit/toggle controls, Git checkpoint AI-change metadata, checkpoint-bound raw Git diff review with session accept/reject decisions, saved metadata-only review artifacts, checkpoint-bound Git approval creation, and direct checkpoint-bound Git run controls, but deeper full-chat execution semantics, actual AI-change file apply/revert workflows, broader editable settings/policy UX beyond CLI and hook policy, VS Code extension, and dedicated CLI client still need to be built.
