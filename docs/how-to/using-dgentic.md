...
Approved non-CLI approval reviews now show editable bound execution request panels instead of a direct CLI-style execute button. Filesystem, web retrieval, provider, and tool approvals require their normal execution endpoint plus the approved `approval_id` and matching request fields; the dashboard shows the endpoint, a safe payload scaffold, editable JSON, binding validation, execution output, and approval refresh. Provider/tool network approvals are handoff-only because they must be consumed as `network_approval_id` by the matching provider or tool execution request.
...
- The first same-origin dashboard is implemented, including a first task-chat composer/transcript with capped local history, actionable task plan/run cards, structured approval review summaries, editable non-CLI bound execution request panels, read-only settings/policy review summaries, local CLI policy rule creation/edit/toggle controls, Git checkpoint AI-change metadata, checkpoint-bound raw Git diff review with session accept/reject decisions, checkpoint-bound Git approval creation, and direct checkpoint-bound Git run controls, but full unified chat, persistent AI-change artifact apply/revert workflows, richer type-specific non-CLI editors, broader editable settings/policy UX, VS Code extension, and dedicated CLI client still need to be built.
