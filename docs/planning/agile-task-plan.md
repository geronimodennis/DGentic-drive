...
Current implementation status:
- Partially implemented: web frontend/dashboard and interactive approval UI are active in Sprint 16, with dashboard shell, actionable task plan/run cards, task planning, project context/open controls, workspace file browsing/editing, guided orchestration creation, orchestration detail/execution controls, expandable per-task sub-agent briefs, parent-child agent graph visibility, task update/recovery/blocker resolution/closeout controls, unified approval source/status filtering plus structured review/decision actions, approved CLI execution, non-CLI bound execution request handoff panels, command recipe actions, Git checkpoint visibility with AI-change metadata summary, checkpoint-bound raw Git diff review, checkpoint-bound commit/push/PR approval creation, direct checkpoint-bound Git run controls, memory/tool reliability summaries, policy/plugin visibility with source/status summaries, local CLI policy rule creation/edit/toggle controls, grouped effective-settings review, and logs implemented.
- Not yet implemented: full chat UX beyond task plan/run cards, AI-change accept/reject artifact workflow beyond raw Git diff visibility, full non-CLI request editors/execution, broader editable settings/policy UX, VS Code extension, and dedicated CLI client.
