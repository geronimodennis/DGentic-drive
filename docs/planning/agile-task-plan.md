...
- Sprint 16 BL-010bx update: Git change-review rationale notes are implemented through checkpoint-bound raw diff review and metadata-only review artifact contracts, with bounded secret-redacted per-section notes and matching-checkpoint browser restore.
- Sprint 16 BL-010by update: Task Chat memory context controls are implemented through existing memory metadata and hybrid retrieval routes; active memory cards, detail rows, and retrieval results can insert bounded memory context into the composer.
- Sprint 16 BL-010bz update: Task Chat provider message-role selection is implemented through the existing provider message role contract; both Task Chat provider replies and Task Chat provider approval requests now use the selected supported role instead of hardcoding `user`.
- Sprint 16 BL-010ca update: Task Chat approval outcome handoff is implemented through existing safe review, decision, CLI execution, and bound execution flows; bounded outcome cards can insert follow-up context or reopen the exact safe review.
- Sprint 16 BL-010cb update: Task Chat provider route preview is implemented through the existing routing decision contract; bounded Provider Route cards can apply the selected provider/model or insert route context without adding provider execution authority.
- Sprint 16 BL-010cc update: Activity/session workbench controls are implemented through the existing `/sessions/summary` and `/logs` contracts; operators can capture/list summaries, filter session logs, and insert bounded session-summary context into Task Chat.
- Sprint 16 BL-010cd update: Provider routing settings review is implemented through the existing `/settings/effective` contract; configured `provider_role_routing` entries render as role cards that can seed Task Chat provider controls or the Providers routing preview role, open the target panels, and avoid changing backend policy.
- Sprint 16 BL-010ce update: Activity log context handoff is implemented through the existing `/logs` contract; filtered log rows can insert bounded Task Chat context or copy safe log evidence without mutating log state.
- Sprint 16 BL-010cf update: Task Chat handoff packets are implemented client-side from existing context/transcript state; operators can preview a bounded continuation packet and copy Markdown or JSON without backend mutation or new execution authority.
- Sprint 16 BL-010cg update: Task Chat Provider Route cards can explicitly Use Route & Ask, applying the selected provider/model and invoking the existing guarded Task Chat provider reply flow without treating route preview as authorization.
- Sprint 16 BL-010ch update: Task Chat Approval Outcome cards can explicitly Use Outcome & Ask, inserting bounded non-capability outcome context and invoking the existing guarded Task Chat provider reply flow without exposing raw outcome IDs or treating them as provider approval IDs.
- Sprint 16 BL-010ci update: Task Chat provider prompt preview is implemented through client-side prompt composition; Preview Prompt appends a bounded, secret-redacted transcript card and does not call provider generation, streaming generation, or approval routes.
- Sprint 16 BL-010cj update: Task Chat Provider Reply and Provider Stream cards can explicitly Use Response & Ask, inserting bounded secret-redacted provider response context and invoking the existing guarded Task Chat provider reply flow without creating approvals or changing backend authority.
- Sprint 16 BL-010ck update: Task Chat plan, run, orchestration, session-summary, active-memory, and log context cards can explicitly Use Context & Ask, inserting bounded secret-redacted context and invoking the existing guarded provider reply flow without creating approvals or treating context as capability authority.
- Sprint 16 BL-010cl update: loaded approval reviews can explicitly Use Review Context or Use Review & Ask with curated safe summary fields, fingerprinted approval references, and no raw approval IDs, approval digests, or bound execution payloads in provider prompts.
- Sprint 16 BL-010cm update: Task Chat composer context review/removal controls are implemented through client-side counts, bounded display-only redacted preview, explicit full-context redaction, and Clear Context before provider asks.
- Sprint 16 BL-010cn update: Task Chat context block management is implemented through client-side blank-line-delimited block parsing, redacted block rows, and explicit per-block removal while preserving remaining composer context order.
- Sprint 16 BL-010co update: Task Chat context block ordering is implemented through client-side Move Up/Move Down controls that reorder blank-line-delimited composer context blocks and refresh stale rendered controls instead of mutating changed context.
- Not yet implemented: richer unified chat semantics beyond deterministic task execution and explicit route/outcome/response/context/review-to-reply handoffs plus non-generating prompt previews, composer context review/redaction/clear controls, and current context block removal/ordering controls, more granular active context organization beyond the blank-line block model and bounded cards, broader approval-response automation beyond curated review/outcome context, actual Git hunk/patch apply or revert workflows beyond guarded workspace editor mutations, richer AI-change review semantics beyond saved metadata-only review artifacts, UI-side diff decisions, and reviewer rationale notes, broader editable settings/policy UX beyond the currently implemented project metadata, read-only provider routing review, CLI policy, hook policy, network policy, command recipes, plugin trust/activation, generated-tool governance, memory lifecycle/compression administration beyond active metadata context insertion, manual thresholded preview/apply, and metadata quick-edit controls, VS Code extension, and dedicated CLI client.
