...
Current implementation status:
- Partially implemented: web frontend/dashboard and interactive approval UI are active in Sprint 16, with dashboard shell, first task-chat composer/transcript with capped browser-local history, task-chat context stream, reusable plan/run follow-up context controls, actionable task plan/run cards, task planning, project context/open controls, workspace file browsing/editing, guided orchestration creation, orchestration detail/execution controls, expandable per-task sub-agent briefs, parent-child agent graph visibility, task update/recovery/blocker resolution/closeout controls, unified approval source/status filtering plus structured review/decision actions, approved CLI execution, guided editable non-CLI bound execution request panels with recursive nested payload controls, command recipe actions plus local command recipe creation/edit/toggle controls, local plugin trust/block controls, network policy preflight checks and approval requests, provider health checks, provider routing preview, generated-tool governance controls, Git checkpoint visibility with AI-change metadata summary, checkpoint-bound raw Git diff review, session accept/reject decisions for loaded Git diff sections, persistent metadata-only AI-change review artifacts, checkpoint-bound commit/push/PR approval creation, direct checkpoint-bound Git run controls, memory/tool reliability summaries, read-only hybrid memory retrieval, memory lifecycle preview, memory compression preview, read-only memory/tool detail drilldowns, policy/plugin visibility with source/status summaries, local CLI and hook policy rule creation/edit/toggle controls, grouped effective-settings review, and logs implemented.
- Not yet implemented: deeper full-chat execution semantics beyond the context stream and reusable plan/run evidence controls, actual AI-change file apply/revert workflow beyond saved review artifacts, broader editable settings/policy UX beyond CLI policy, hook policy, command recipes, plugin trust, and generated-tool governance, memory lifecycle/compression apply actions beyond read-only preview/detail surfaces, VS Code extension, and dedicated CLI client.
