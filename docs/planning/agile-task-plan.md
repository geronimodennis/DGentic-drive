...
- Not yet implemented: richer unified chat semantics beyond deterministic task execution and explicit orchestration creation, deeper Git hunk/patch apply or revert workflows beyond guarded workspace editor mutations and saved metadata-only review artifacts, broader editable settings/policy UX beyond the currently implemented CLI policy, hook policy, command recipes, plugin trust, generated-tool governance, broader memory lifecycle/compression management beyond apply controls, VS Code extension, and dedicated CLI client.
