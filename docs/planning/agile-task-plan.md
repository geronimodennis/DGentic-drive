...
- Not yet implemented: richer unified chat semantics beyond deterministic task execution, explicit orchestration creation, reusable orchestration context, and approval-review handoff, actual Git hunk/patch apply or revert workflows beyond guarded workspace editor mutations, richer AI-change review semantics beyond saved metadata-only review artifacts and UI-side diff decisions, broader editable settings/policy UX beyond the currently implemented CLI policy, hook policy, network policy, command recipes, plugin trust, generated-tool governance, memory lifecycle/compression administration beyond manual thresholded preview/apply controls, VS Code extension, and dedicated CLI client.
