...
- Remaining: deeper conversational history beyond the first task-chat composer, persistent AI-change artifact apply/revert workflows beyond session review annotations, richer type-specific non-CLI request editors and provider/tool network-approval consuming flows, broader editable policy/settings surfaces for providers, routing, filesystem, CLI policy lifecycle, memory, tools, command recipes, plugins, hook policies, and managed settings, broader responsive/browser QA, and end-to-end approval scenario tests.
...
- Remaining: deepen the UI into richer conversational history beyond the first task-chat composer, persistent AI-change artifact apply/revert workflows, broader editable policy/settings surfaces, richer type-specific non-CLI editors, persistent/multi-worker project activation semantics, end-to-end approval scenario coverage, and broader browser/responsive validation.
...
- Interactive approval UI: BL-010, Sprint 16. BL-010a unified approval inbox with review plus approve/deny actions, BL-010c approved CLI execution, BL-010i source/status filtering, BL-010u structured safe review summaries, BL-010y non-CLI bound execution handoff panels, and BL-010ab editable bound request execution are implemented; richer type-specific non-CLI editors, provider/tool network-approval consuming flows, and end-to-end approval scenario coverage remain.
