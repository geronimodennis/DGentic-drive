...
- Remaining: deepen the UI into full-chat execution semantics beyond the context stream, actual AI-change file apply/revert mutation workflows beyond saved review artifacts, broader editable policy/settings surfaces beyond CLI policy, hook policy, and command recipes, and persistent/multi-worker project activation semantics.
