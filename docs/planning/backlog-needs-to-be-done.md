...
- Remaining: full unified chat beyond local task history, persistent AI-change artifact apply/revert workflows beyond session review annotations, richer type-specific non-CLI request editors and provider/tool network-approval consuming flows, broader editable policy/settings surfaces for providers, routing, filesystem, CLI policy lifecycle, memory, tools, command recipes, plugins, hook policies, and managed settings, and end-to-end approval scenario tests.
...
- Remaining: deepen the UI into full unified chat beyond local task history, persistent AI-change artifact apply/revert workflows, broader editable policy/settings surfaces, richer type-specific non-CLI editors, persistent/multi-worker project activation semantics, and end-to-end approval scenario coverage.
