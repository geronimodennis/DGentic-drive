...
- Remaining: deeper conversational history beyond the first task-chat composer, persistent AI-change artifact apply/revert workflows beyond session review annotations, full non-CLI bound request editors/execution, broader editable policy/settings surfaces for providers, routing, filesystem, CLI policy lifecycle, memory, tools, command recipes, plugins, hook policies, and managed settings, broader responsive/browser QA, and end-to-end approval scenario tests.
...
- Remaining: deepen the UI into richer conversational history beyond the first task-chat composer, persistent AI-change artifact apply/revert workflows, broader editable policy/settings surfaces, full non-CLI bound request editors/execution UX, persistent/multi-worker project activation semantics, end-to-end approval scenario coverage, and broader browser/responsive validation.
