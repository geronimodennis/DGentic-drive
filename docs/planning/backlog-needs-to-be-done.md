...
- Remaining: deepen the UI into full-chat execution semantics beyond the context stream and reusable plan/run evidence controls, actual AI-change file apply/revert mutation workflows beyond saved review artifacts, broader editable policy/settings surfaces beyond CLI policy, hook policy, command recipes, plugin trust, and generated-tool governance, memory lifecycle/compression apply actions beyond preview/detail surfaces, and persistent/multi-worker project activation semantics.
