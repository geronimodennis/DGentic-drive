...
- Active: BL-010a same-origin dashboard shell, BL-010b current-root workspace file browser/editor, BL-010c CLI approval execution/run-output visibility, BL-010d read-only policy/plugin visibility, and BL-010e active root context plus structured Git checkpoint review are implemented and validation-clean. These slices provide the first user-facing UI surface for task planning, workspace file browsing/editing inside the configured `rootDir`, active root context visibility, orchestration summary, approval review/decisions, approved CLI execution, CLI run output inspection, structured Git checkpoint blockers/warnings/diff-stat review, provider/tool summaries, policy/plugin visibility, effective settings, and logs while preserving backend approval/auth/filesystem/CLI/policy boundaries.
- Remaining: deepen the UI into richer chat/task execution workflows, real project registry/add/open and active `rootDir` switching backend contracts, Codex-style AI-change review, sub-agent progress detail, editable policy/settings surfaces, memory/tool reliability dashboards, richer approval filtering/non-CLI execution UX, and broader browser/responsive validation.
...
- Web frontend/dashboard: BL-010, Sprint 16. BL-010a same-origin `/ui/` dashboard shell, BL-010b current-root workspace file browser/editor, BL-010c CLI approval execution/run-output visibility, BL-010d read-only policy/plugin visibility, and BL-010e active root context plus structured Git checkpoint review are implemented; richer dashboard, chat, real project add/open/rootDir switching, AI-change review, editable settings/policy, memory/tool, and responsive/browser coverage remains.
