...
- Remaining: task-chat to orchestration-run handoff and richer unified chat semantics, deeper Git hunk/patch apply or revert workflows beyond guarded workspace editor mutations and metadata-only review artifacts, broader editable policy/settings surfaces for providers, routing, and filesystem policy, richer CLI policy lifecycle, deeper memory lifecycle/compression administration beyond the new guarded apply controls, deeper tools/plugins workflows beyond governance and trust, and persistent/multi-worker project activation semantics.
...
- Remaining: task-chat to orchestration-run handoff and richer unified chat semantics, deeper Git hunk/patch apply or revert workflows beyond guarded workspace editor mutations and metadata-only review artifacts, broader editable policy/settings surfaces beyond the currently implemented CLI policy, hook policy, command recipes, plugin trust, generated-tool governance, broader memory lifecycle/compression management beyond apply controls, and persistent/multi-worker project activation semantics.
