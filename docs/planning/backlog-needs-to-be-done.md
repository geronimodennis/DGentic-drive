...
- Active: BL-010a same-origin dashboard shell, BL-010b current-root workspace file browser/editor, BL-010c CLI approval execution/run-output visibility, BL-010d read-only policy/plugin visibility, BL-010e active root context plus structured Git checkpoint review, BL-010f project registry/root preflight, BL-010g safe registered-project activation, BL-010h richer orchestration console, and BL-010i approval source/status filtering are implemented and validation-clean. These slices provide the first user-facing UI surface for task planning, workspace file browsing/editing inside the active `rootDir`, active root context visibility, project root preflight, registration, and guarded Open flows, orchestration summary/detail with task graph and execution controls, approval review/decisions/filtering, approved CLI execution, CLI run output inspection, structured Git checkpoint blockers/warnings/diff-stat review, provider/tool summaries, policy/plugin visibility, effective settings, and logs while preserving backend approval/auth/filesystem/CLI/policy boundaries.
- Remaining: deepen the UI into richer chat/task execution workflows, Codex-style AI-change review, sub-agent progress detail, editable policy/settings surfaces, memory/tool reliability dashboards, non-CLI execution UX, persistent/multi-worker project activation semantics, and broader browser/responsive validation.
