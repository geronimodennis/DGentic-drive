...
- Active: BL-010a same-origin dashboard shell, BL-010b current-root workspace file browser/editor, BL-010c CLI approval execution/run-output visibility, BL-010d read-only policy/plugin visibility, BL-010e active root context plus structured Git checkpoint review, and BL-010f project registry/root preflight are implemented and validation-clean. These slices provide the first user-facing UI surface for task planning, workspace file browsing/editing inside the configured `rootDir`, active root context visibility, project root preflight and registration, orchestration summary, approval review/decisions, approved CLI execution, CLI run output inspection, structured Git checkpoint blockers/warnings/diff-stat review, provider/tool summaries, policy/plugin visibility, effective settings, and logs while preserving backend approval/auth/filesystem/CLI/policy boundaries.
- Remaining: deepen the UI into richer chat/task execution workflows, safe active `rootDir` switching for registered projects, Codex-style AI-change review, sub-agent progress detail, editable policy/settings surfaces, memory/tool reliability dashboards, richer approval filtering/non-CLI execution UX, and broader browser/responsive validation.
...
- Web frontend/dashboard: BL-010, Sprint 16. BL-010a same-origin `/ui/` dashboard shell, BL-010b current-root workspace file browser/editor, BL-010c CLI approval execution/run-output visibility, BL-010d read-only policy/plugin visibility, BL-010e active root context plus structured Git checkpoint review, and BL-010f project registry/root preflight are implemented; richer dashboard, chat, active rootDir switching, AI-change review, editable settings/policy, memory/tool, and responsive/browser coverage remains.
- VS Code extension: BL-011, Sprint 17.
- Dedicated CLI client interface: BL-011, Sprint 17.
- Interactive approval UI: BL-010, Sprint 16. BL-010a unified approval inbox with review plus approve/deny actions and BL-010c approved CLI execution are implemented; non-CLI execute flows, richer filtering/detail, and end-to-end approval scenario coverage remains.
