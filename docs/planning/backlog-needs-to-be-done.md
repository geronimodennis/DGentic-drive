...
- Active: BL-010a same-origin dashboard shell, BL-010b current-root workspace file browser/editor, and BL-010c CLI approval execution/run-output visibility are implemented and validation-clean. These slices provide the first user-facing UI surface for task planning, workspace file browsing/editing inside the configured `rootDir`, orchestration summary, approval review/decisions, approved CLI execution, CLI run output inspection, Git checkpoints, provider/tool summaries, effective settings, and logs while preserving backend approval/auth/filesystem/CLI boundaries.
...
- Web frontend/dashboard: BL-010, Sprint 16. BL-010a same-origin `/ui/` dashboard shell, BL-010b current-root workspace file browser/editor, and BL-010c CLI approval execution/run-output visibility are implemented; richer dashboard, chat, project add/open/rootDir switching, AI-change review, settings, policy, memory/tool, and responsive/browser coverage remains.
