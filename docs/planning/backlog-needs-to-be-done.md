...
- Remaining: richer production identity workflows beyond persisted operators and operator groups, managed KMS integration beyond supplied-key local vault rotation, first-class external secret manager adapters beyond the generic process-adapter bridge, managed policy-source controls beyond CLI/hook/command-recipe/plugin-trust/plugin-component records and coarse surface locks, and OS-level/non-Python generated-tool egress isolation beyond the current Python socket guardrail.
...
- Network/domain guardrails: provider-call network policy is completed under BL-009/Sprint 15 BL-009c, provider-call network approval records are completed under BL-009h, generated-tool Python socket policy enforcement is completed under BL-009j, the web retrieval network guard contract is completed under BL-009ai, generated-tool network approval consumption is completed under BL-009am, and the guarded web retrieval fetch runtime is completed under BL-009ap. OS-level/non-Python egress isolation remains under BL-009/Sprint 15 follow-up.
