...
- Moved to follow-up backlog: interactive filesystem approval UI, persisted configurable filesystem policy rules, deeper platform-specific locked-file validation, and OS-level filesystem isolation.
...
- Remaining: deeper full-chat execution semantics beyond the context stream, actual AI-change file apply/revert mutation workflows beyond saved review artifacts, broader editable policy/settings surfaces for providers, routing, filesystem beyond guardrail preflight, richer CLI policy lifecycle, deeper memory lifecycle/compression administration beyond the new guarded apply controls, deeper tools/plugins workflows beyond governance and trust, and persistent/multi-worker project activation semantics.
...
- BL-010be update: filesystem guardrail preflight is implemented and validation-clean; the dashboard can check filesystem action decisions through the existing `/guardrails/filesystem` contract without executing filesystem operations or creating approval bypasses.
- Remaining: deepen the UI into full-chat execution semantics beyond the context stream and reusable plan/run evidence controls, actual AI-change file apply/revert mutation workflows beyond saved review artifacts, broader editable policy/settings surfaces beyond the currently implemented CLI policy, hook policy, command recipes, plugin trust, generated-tool governance, memory lifecycle/compression apply controls, and filesystem guardrail preflight, and persistent/multi-worker project activation semantics.
