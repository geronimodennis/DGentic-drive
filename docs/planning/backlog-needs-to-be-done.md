...
- Remaining after BL-009d: richer user/group identity workflows, encrypted local credential vaulting or external secret-manager adapters beyond env references, network approval records and non-provider network surfaces, broader audit actor propagation, and cross-surface no-secret-response validation.
...
- Full production identity management, secret management, encrypted credential storage, and token rotation: BL-009, Sprint 15. Persisted operator profiles and generated token lifecycle APIs are complete; richer identity workflows and encrypted/secret-manager storage remain.
