...
- BL-010ca update: Task Chat approval outcome handoff is implemented and validation-clean; approval decisions, CLI approval execution, and dashboard-callable bound execution can append bounded outcome cards with Use Outcome and exact Review controls.
- BL-010cb update: Task Chat provider route preview is implemented and validation-clean; the task-chat composer can call the existing `/routing/decide` contract, append bounded Provider Route cards, apply the chosen provider/model to chat provider controls, and insert route context without adding provider execution authority.
- BL-010cc update: Activity and Session Workbench is implemented and validation-clean; the dashboard can capture/list session summaries through existing session APIs, filter session logs, and insert bounded session-summary context into Task Chat.
- BL-010cd update: Provider routing settings review is implemented and validation-clean; the Settings panel parses configured `provider_role_routing` from effective settings and can seed existing Task Chat provider/model controls or Providers routing-preview role input while opening the target panels.
- BL-010ce update: Activity log context handoff is implemented and validation-clean; filtered Activity log rows can insert bounded event context into Task Chat or copy safe log evidence.
- BL-010cf update: Task Chat handoff packet preview/copy is implemented and validation-clean; operators can generate bounded Markdown or JSON continuation evidence from existing Task Chat context, transcript, Activity logs, approvals, and provider route/reply metadata.
- BL-010cg update: Task Chat route-to-reply automation is implemented and validation-clean; Provider Route cards can explicitly Use Route & Ask by applying the route and invoking the existing guarded provider reply flow while leaving authorization to `/providers/generate` or `/providers/generate/stream`.
- BL-010ch update: Task Chat approval outcome-to-reply automation is implemented and validation-clean; Approval Outcome cards can explicitly Use Outcome & Ask by inserting bounded non-capability outcome context before invoking the existing guarded provider reply flow without exposing raw outcome IDs or injecting them as provider approval IDs.
- BL-010ci update: Task Chat provider prompt preview is implemented and validation-clean; Preview Prompt appends a bounded, secret-redacted provider prompt transcript card without calling provider generation, streaming generation, or approval routes.
- BL-010cj update: Task Chat provider response-to-reply automation is implemented and validation-clean; Provider Reply and Provider Stream cards can explicitly Use Response & Ask by inserting bounded, secret-redacted response context before invoking the existing guarded provider reply flow without creating approvals or changing backend authority.
- BL-010ck update: Task Chat context-to-reply automation is implemented and validation-clean; plan, run, orchestration, session-summary, active-memory, and log context cards can explicitly Use Context & Ask by inserting bounded, secret-redacted context before invoking the existing guarded provider reply flow without creating approvals or treating context as capability authority.
- BL-010cl update: Safe approval review context-to-reply automation is implemented and validation-clean; loaded approval reviews can Use Review Context or Use Review & Ask with allowlisted safe review fields, fingerprinted approval references, and no raw approval IDs, approval digests, or bound execution payloads in provider prompts.
- BL-010cm update: Task Chat context review/removal is implemented and validation-clean; the composer shows context counts, a bounded display-only redacted preview, explicit full-context redaction that preserves long non-secret content, and Clear Context before provider asks.
- Remaining: richer unified chat semantics beyond deterministic execution and explicit route/outcome/response/context/review-to-reply handoffs plus non-generating prompt previews and composer context review/redaction/clear controls, more granular active context organization beyond bounded cards and composer-level review, broader approval-response automation beyond curated review/outcome context, actual Git hunk/patch apply or revert workflows beyond guarded workspace editor mutations, richer AI-change review semantics beyond metadata-only review artifacts, UI-side diff decisions, and reviewer rationale notes, broader editable policy/settings surfaces beyond the currently implemented project metadata, read-only provider routing review, CLI policy, hook policy, network policy, command recipes, plugin trust/activation, generated-tool governance, memory lifecycle/compression administration beyond active metadata context insertion, manual thresholded preview/apply, and metadata quick-edit controls, and persistent/multi-worker project activation semantics.
...
- Task Chat provider replies: BL-010bv and BL-010cj, Sprint 16. Implemented Ask Provider controls in Task Chat for non-streaming and streaming provider replies through existing guarded provider routes, including provider/model population, optional bound approvals, transcript reply cards, bounded context insertion, and explicit Use Response & Ask follow-up through redacted response context.
...
- Task Chat approval outcomes: BL-010ca and BL-010ch, Sprint 16. Implemented bounded approval outcome cards after approval decisions and executions, including Use Outcome context insertion with non-capability approval references, exact safe-review reopening, and explicit Use Outcome & Ask through the existing guarded provider reply path without treating outcome IDs as provider approval IDs.
- Task Chat provider route preview and route-to-reply: BL-010cb and BL-010cg, Sprint 16. Implemented Task Chat route preview cards through `/routing/decide`, with Use Route provider/model application, explicit Use Route & Ask through the existing guarded provider reply path, and Use Context insertion while leaving authorization to the provider generation contracts.
- Task Chat provider prompt preview: BL-010ci, Sprint 16. Implemented Preview Prompt in Task Chat provider controls, rendering the exact composed provider prompt as bounded, secret-redacted transcript context without provider execution, streaming execution, approval creation, or backend route changes.
- Task Chat provider response-to-reply: BL-010cj, Sprint 16. Implemented Use Response & Ask on Provider Reply and Provider Stream cards, inserting bounded redacted response context before invoking the existing guarded provider reply path without automatic chaining or approval creation.
- Task Chat context review/removal: BL-010cm, Sprint 16. Implemented composer context counts, bounded display-only redacted preview, explicit full-context redaction, and Clear Context controls so operators can inspect or sanitize active context before provider asks without backend mutation.
- Activity/session workbench: BL-010cc and BL-010ce, Sprint 16. Implemented session summary capture/list/use-context controls through `/sessions/summary`, session event filtering through `/logs`, bounded session-summary context cards in Task Chat, and Activity log Use Context/Copy Evidence controls for filtered log rows.
- Task Chat handoff packet: BL-010cf, Sprint 16. Implemented a client-side Markdown/JSON handoff packet preview that bundles bounded composer, active root, recent context, Activity log/session evidence, approval references, provider route/reply metadata, and transcript summaries without backend mutation.
- Provider routing settings review: BL-010cd, Sprint 16. Implemented parsed `provider_role_routing` cards in Settings with Task Chat and Providers preview handoff controls while leaving settings mutation to deployment-managed configuration.
