...
- Remaining: richer chat/task execution workflows beyond the console, accept/reject AI-change artifact workflows beyond raw Git diff visibility, full non-CLI bound request editors/execution, broader editable policy/settings surfaces for providers, routing, filesystem, CLI policy lifecycle, memory, tools, command recipes, plugins, hook policies, and managed settings, broader responsive/browser QA, and end-to-end approval scenario tests.
...
- Remaining: deepen the UI into full chat workflows beyond the task-plan cards, accept/reject AI-change artifact workflows, broader editable policy/settings surfaces, full non-CLI bound request editors/execution UX, persistent/multi-worker project activation semantics, end-to-end approval scenario coverage, and broader browser/responsive validation.
...
- Web frontend/dashboard: BL-010, Sprint 16. BL-010a same-origin `/ui/` dashboard shell, BL-010b current-root workspace file browser/editor, BL-010c CLI approval execution/run-output visibility, BL-010d read-only policy/plugin visibility, BL-010e active root context plus structured Git checkpoint review, BL-010f project registry/root preflight, BL-010g safe in-process project activation/rootDir switching, BL-010h richer orchestration console, BL-010i approval filtering, BL-010j per-task sub-agent briefs, BL-010k orchestration recovery/closeout controls, BL-010l orchestration creation, BL-010m sub-agent graph visibility, BL-010n memory/tool reliability dashboards, BL-010o guided task graph builder, BL-010p command recipe actions, BL-010q read-only settings/policy/Git review summaries, BL-010r CLI policy rule creation UI, BL-010s CLI policy rule edit/toggle UI, BL-010t Git checkpoint approval actions, BL-010u structured approval review summaries, BL-010v checkpoint-bound raw Git diff review, and BL-010w checkpoint review-to-run Git actions are implemented; richer dashboard, chat, accept/reject AI-change artifact workflows, broader editable settings/policy, persistent or multi-worker project activation, and responsive/browser coverage remains.
- Web task planner/run UI: BL-010x, Sprint 16. Implemented actionable task plan cards, plan-step detail, context chips, related deterministic run history, and Run Plan execution through `/tasks/execute`; full chat workflows remain.
- VS Code extension: BL-011, Sprint 17.
- Dedicated CLI client interface: BL-011, Sprint 17.
- Interactive approval UI: BL-010, Sprint 16. BL-010a unified approval inbox with review plus approve/deny actions, BL-010c approved CLI execution, BL-010i source/status filtering, BL-010u structured safe review summaries, and BL-010y non-CLI bound execution handoff panels are implemented; full non-CLI request editors/execution and end-to-end approval scenario coverage remain.
