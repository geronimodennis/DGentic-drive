...
- Remaining: richer unified chat semantics beyond deterministic execution, explicit orchestration creation, reusable orchestration context, approval-review handoff, and provider response context handoff, actual Git hunk/patch apply or revert workflows beyond guarded workspace editor mutations, richer AI-change review semantics beyond metadata-only review artifacts and UI-side diff decisions, broader editable policy/settings surfaces beyond the currently implemented project metadata, CLI policy, hook policy, network policy, command recipes, plugin trust/activation, generated-tool governance, memory lifecycle/compression administration beyond manual thresholded preview/apply and metadata quick-edit controls, and persistent/multi-worker project activation semantics.
