...
- Remaining: deeper conversational history beyond the first task-chat composer, accept/reject AI-change artifact workflows beyond raw Git diff visibility, full non-CLI bound request editors/execution, broader editable policy/settings surfaces for providers, routing, filesystem, CLI policy lifecycle, memory, tools, command recipes, plugins, hook policies, and managed settings, broader responsive/browser QA, and end-to-end approval scenario tests.
