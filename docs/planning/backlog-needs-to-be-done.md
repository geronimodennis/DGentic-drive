...
- Remaining: richer production identity workflows beyond persisted operators and operator groups, managed KMS integration beyond supplied-key local vault rotation, first-class external secret manager adapters beyond the generic process-adapter bridge, web retrieval network enforcement, managed policy-source controls beyond CLI/hook policy rules and coarse surface locks, and OS-level/non-Python generated-tool egress isolation beyond the current Python socket guardrail.
