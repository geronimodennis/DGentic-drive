...
- Active: BL-010a same-origin dashboard shell, BL-010b current-root workspace file browser/editor, BL-010c CLI approval execution/run-output visibility, BL-010d read-only policy/plugin visibility, BL-010e active root context plus structured Git checkpoint review, BL-010f project registry/root preflight, BL-010g safe registered-project activation, BL-010h richer orchestration console, BL-010i approval source/status filtering, BL-010j per-task sub-agent briefs, BL-010k orchestration recovery/closeout controls, BL-010l orchestration run creation, BL-010m sub-agent graph visibility, and BL-010n memory/tool reliability dashboards are implemented and validation-clean. These slices provide the first user-facing UI surface for task planning, workspace file browsing/editing inside the active `rootDir`, active root context visibility, project root preflight, registration, and guarded Open flows, orchestration creation, orchestration summary/detail with task graph, expandable agent brief detail, parent-child agent graph detail, task update/recovery/blocker resolution/closeout controls, execution controls, memory/tool reliability summaries, approval review/decisions/filtering, approved CLI execution, CLI run output inspection, structured Git checkpoint blockers/warnings/diff-stat review, provider/tool summaries, policy/plugin visibility, effective settings, and logs while preserving backend approval/auth/filesystem/CLI/policy boundaries.
- Remaining: deepen the UI into richer chat/task execution workflows, Codex-style AI-change review, guided task graph builders, editable policy/settings surfaces, non-CLI execution UX, persistent/multi-worker project activation semantics, and broader browser/responsive validation.
