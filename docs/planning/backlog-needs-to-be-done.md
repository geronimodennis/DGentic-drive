...
- Remaining: full unified chat beyond local task history, persistent AI-change artifact apply/revert workflows beyond session review annotations, deeper nested type-specific non-CLI editors, broader editable policy/settings surfaces for providers, routing, filesystem, CLI policy lifecycle, memory, tools, command recipes, plugins, hook policies, and richer browser scenarios for generated-tool network-approval consumption.
