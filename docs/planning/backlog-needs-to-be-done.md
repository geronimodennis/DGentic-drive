...
- Remaining: richer production identity workflows beyond persisted operators and operator groups, managed KMS integration beyond supplied-key local vault rotation, first-class external secret manager adapters beyond the generic process-adapter bridge, web retrieval network enforcement, managed policy-source controls beyond CLI policy rules and coarse surface locks, and OS-level/non-Python generated-tool egress isolation beyond the current Python socket guardrail.
...
- Full production identity management, secret management, encrypted credential storage, and token rotation: BL-009, Sprint 15. Persisted operator profiles, persisted operator groups with capability inheritance, generated token lifecycle APIs, identity/token/credential/plugin-trust metadata redaction, operator-supplied local encrypted vault references with supplied-key rotation, generic external-process credential adapter plumbing, plugin manifest trust controls, plugin command recipe activation governance, backend hook-policy records, bound filesystem approval records, managed settings precedence foundation, managed policy surface locks, managed CLI policy rule precedence, command recipe execution contracts, read-only git workflow safety checkpoints, checkpoint-bound git commit/push/PR approval creation, and active-task verification for caller-supplied orchestration agent context are complete; richer identity workflows beyond operator groups, managed KMS integration, first-class secret-manager adapters, plugin hook/tool/agent/skill loading governance, managed policy-source controls beyond CLI policy rules, and direct git/PR workflow runners beyond approval-bound CLI execution remain.
