...
- Latest stable implementation checkpoint: BL-010g safe project activation/rootDir switching is implemented and full-regression clean.
- Latest completed slice: BL-010g, admin-gated registered-project activation with in-process runtime `rootDir` switching, anchored DGentic state, active-run/approval/orchestration blockers, dashboard Open controls, focused route/UI/auth regression coverage, and live local UI/API smoke validation.
- Current objective: continue deepening the Sprint 16 user-facing UI, with AI-change review, richer settings editors, richer approval filtering/non-CLI execution UX, and broader browser validation next, while keeping remaining Sprint 15 backend security and Git expansion work deferred, not cancelled.
...
- Sprint 16: cross-platform UI and approval dashboard, including chat, project add/open, file explorer, code editor, AI-change review, Git checkpoint, approval, run history, blocker, and freshness surfaces. Active with BL-010a through BL-010g implemented.
