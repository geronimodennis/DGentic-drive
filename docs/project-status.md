...
- Latest stable implementation checkpoint: BL-010ap local hook policy editor UI is implemented and validation-clean.
- Latest completed slice: BL-010ap, the dashboard can create, edit, and enable/disable local hook policy rules through existing guarded hook-policy APIs, keeps managed/plugin rules read-only, respects managed `hook_policy` locks, and validates non-`any` match patterns before submission.
- Current objective: continue deepening the Sprint 16 user-facing UI, with actual AI-change file apply/revert mutation workflows beyond saved review artifacts, broader editable settings and policy workflows beyond CLI and hook policy, full unified chat beyond local task history, and persistent or multi-worker project activation semantics, while keeping remaining Sprint 15 backend security and Git expansion work deferred, not cancelled.
