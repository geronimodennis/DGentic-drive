...
- Latest stable implementation checkpoint: BL-010av provider health check UI is implemented and validation-clean.
- Latest completed slice: BL-010av, the dashboard Provider panel can now run per-provider health checks through the existing `/providers/{provider_id}/health` contract and render safe availability/model details.
