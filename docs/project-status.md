...
- Latest stable implementation checkpoint: BL-010aw provider routing preview UI is implemented and validation-clean.
- Latest completed slice: BL-010aw, the dashboard Provider panel can now preview provider/model routing decisions through the existing `/routing/decide` contract without starting provider generation.
