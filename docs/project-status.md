...
- Latest stable implementation checkpoint: BL-010at network policy preflight UI is implemented and validation-clean.
- Latest completed slice: BL-010at, the dashboard Policy view can now check generic and web-retrieval network policy decisions through existing `/guardrails/network` and `/web-retrieval/network/check` contracts without adding mutation authority.
