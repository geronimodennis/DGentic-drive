...
- Latest stable implementation checkpoint: BL-010f project registry and root preflight is implemented and focused-validation clean.
- Latest completed slice: BL-010f, admin-gated project root preflight/register/list/detail/update APIs plus `/ui/` project registry controls that do not activate or switch `rootDir`.
- Current objective: continue deepening the Sprint 16 user-facing UI, with a safe active project/rootDir switching transaction, AI-change review, richer settings editors, and broader browser validation next, while keeping remaining Sprint 15 backend security and Git expansion work deferred, not cancelled.
...
- Sprint 16: cross-platform UI and approval dashboard, including chat, project add/open, file explorer, code editor, AI-change review, Git checkpoint, approval, run history, blocker, and freshness surfaces. Active with BL-010a through BL-010f implemented.
