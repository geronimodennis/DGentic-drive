...
def test_bound_approval_rejects_changed_managed_hook_policy_rule(runtime, monkeypatch) -> None:
    service, _root_dir, data_dir = runtime
    managed_path = data_dir.parent / "managed-settings.json"

    def write_managed_hook(rule_id: str, name: str, reason: str) -> None:
        managed_path.write_text(
            json.dumps(
                {
                    "settings": {
    ...
    write_managed_hook(
        "managed.audit-python.v1",
        "Managed Python audit v1",
        "Initial managed hook audit.",
    )
    ...
    write_managed_hook(
        "managed.audit-python.v2",
        "Managed Python audit v2",
        "Changed managed hook audit.",
    )
    ...
    stored = service.list_approvals()[0]
    assert approval.hook_policy is not None
    assert approval.hook_policy.matched_rule_id == "managed.audit-python.v1"
    assert approval.hook_policy.matched_rule_name == "Managed Python audit v1"
    assert stored.status == CommandApprovalStatus.approved
    assert service.list_command_runs() == []
